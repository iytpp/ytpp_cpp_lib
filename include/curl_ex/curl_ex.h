...
#error "curl_ex v4.2.5 requires libcurl 7.56.0 or newer"
#endif
...
namespace ytpp::curl_ex {
	...
	/// 当前封装版本。
	inline constexpr std::string_view kCurlExVersion = "4.2.5";
	...
	struct TlsOptions {
		...
		TlsVersion minVersion = TlsVersion::Default; ///< 最低 TLS 版本。
	...
	struct RedirectOptions {
		...
		bool allowHttpsToHttp = true;          ///< 是否允许重定向目标使用 HTTP；关闭后所有重定向目标仅允许 HTTPS，可阻止任意重定向链中的 HTTPS→HTTP 降级。
