	...
	int RunServerExample()
	{
		...
		server.RegisterFunction("Add", PermissionLevel::User,
			[](const RpcCallContext&, const RpcArray& args) -> RpcResult
			{
				...
				std::int32_t a = args[0].AsInt32();
				std::int32_t b = args[1].AsInt32();
		...
		server.RegisterFunction("GetPair", PermissionLevel::User,
			[](const RpcCallContext&, const RpcArray&) -> RpcResult
			{
				RpcResult r;
				r.success = true;
				r.returnValues.push_back(RpcValue(std::int32_t(123)));
	...
	int RunClientExample()
	{
...

		{
			...
			bool ok = client.Call("Add",
				{
					RpcValue(std::int32_t(100)),
					RpcValue(std::int32_t(200))
				}, r, err);
			if (ok)
...

		{
			...
			if (ok)
			{
				std::int32_t a = r.returnValues[0].AsInt32();
...

		{
			...
			if (ok)
			{
				UserInfo out = ToUserInfo(r.returnValues[0]);
