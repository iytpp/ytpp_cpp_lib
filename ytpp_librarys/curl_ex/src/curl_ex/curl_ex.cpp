...
class CurlGlobalRuntime {
  public:
    ...
    ~CurlGlobalRuntime() {
        ...
        if (info != nullptr && info->version_num >= 0x075400 && (info->features & CURL_VERSION_THREADSAFE) != 0) {
            curl_global_cleanup();
        }
#endif
    }

    CURLcode GetCode() const noexcept {
        return code_;
    }
...
CurlGlobalRuntime& GetGlobalRuntime() {
    static CurlGlobalRuntime runtime;
    return runtime;
}

bool EnsureGlobal(_Out_ HttpResponse* response = nullptr) {
    const CURLcode code = GetGlobalRuntime().GetCode();
    if (code == CURLE_OK) {
        return true;
    }
    if (response) {
...
std::string StripLineEnding(_In_ std::string_view line) {
...
std::optional<std::chrono::system_clock::time_point> ParseHttpDate(_In_ const std::string& text) {
    std::tm tm{};
...
std::string ToUpperCopy(_In_ std::string_view value) {
    std::string out(value);
    std::transform(out.begin(), out.end(), out.begin(),
...
std::string ActiveTlsBackendNameFromVersionString(_In_ std::string_view sslVersion) {
    // MultiSSL 的 sslVersion 会把未激活 Backend 放在括号内，例如：
...
bool ShouldUseNativeCa(_In_ const TlsOptions& tls) {
#ifdef _WIN32
    // 显式 caFile/caPath 代表调用方希望收窄或替换默认信任来源。
    // CURLSSLOPT_NATIVE_CA 与自定义 CA 是“追加”关系，因此这里主动互斥，避免默认 Native CA
    // 无意扩大显式自定义 CA 的信任集合。
    return tls.verifyPeer && tls.useNativeCa && tls.caFile.empty() && tls.caPath.empty() &&
...
void LogTlsConfigurationDebug(_Inout_ CURL* curl, _In_ const RequestOptions& options) noexcept {
...
long SchannelRevocationOptions(_In_ CertificateRevocationPolicy policy) {
    switch (policy) {
...
bool IsHttpsProxy(_In_ const ProxyOptions& proxy) {
    if (proxy.type) {
...
bool ContainsCrLf(_In_ std::string_view value) noexcept {
    return value.find('\r') != std::string_view::npos || value.find('\n') != std::string_view::npos;
}

bool ContainsNul(_In_ std::string_view value) noexcept {
    return value.find('\0') != std::string_view::npos;
}

bool IsValidHttpVersion(_In_ HttpVersion version) noexcept {
    switch (version) {
...
bool IsValidProxyType(_In_ ProxyType type) noexcept {
    switch (type) {
    case ProxyType::Http:
...
bool IsValidTlsVersion(_In_ TlsVersion version) noexcept {
    switch (version) {
...
bool IsValidRevocationPolicy(_In_ CertificateRevocationPolicy policy) noexcept {
    switch (policy) {
    case CertificateRevocationPolicy::Strict:
    case CertificateRevocationPolicy::BestEffort:
    case CertificateRevocationPolicy::Disabled:
...
int TlsVersionRank(_In_ TlsVersion version) noexcept {
    switch (version) {
    case TlsVersion::Default:
        return 0;
    case TlsVersion::Tls1_0:
...
bool IsHttpToken(_In_ std::string_view value) noexcept {
    if (value.empty())
        return false;
    for (const char ch : value) {
        const unsigned char c = static_cast<unsigned char>(ch);
        const bool alphaNum = (c >= '0' && c <= '9') || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z');
        const bool tchar = alphaNum || ch == '!' || ch == '#' || ch == '$' || ch == '%' || ch == '&' || ch == '\'' ||
...
bool IsSafeHttpFieldValue(_In_ std::string_view value) noexcept {
    for (const char ch : value) {
        const unsigned char c = static_cast<unsigned char>(ch);
...
bool IsValidRawHeaderLine(_In_ std::string_view line) noexcept {
    if (line.empty() || ContainsCrLf(line) || !IsSafeHttpFieldValue(line))
        return false;
    const std::size_t colon = line.find(':');
    if (colon != std::string_view::npos)
        return IsHttpToken(line.substr(0, colon));
    return line.back() == ';' && line.find(';') == line.size() - 1U && IsHttpToken(line.substr(0, line.size() - 1U));
}

bool ContainsCode(_In_ const std::vector<CURLcode>& values, _In_ CURLcode value) {
    return std::find(values.begin(), values.end(), value) != values.end();
}

bool ContainsStatus(_In_ const std::vector<long>& values, _In_ long value) {
    return std::find(values.begin(), values.end(), value) != values.end();
}

bool IsIdempotentMethod(_In_ std::string_view method) {
    const std::string upper = ToUpperCopy(method);
    return upper == "GET" || upper == "HEAD" || upper == "PUT" || upper == "DELETE" || upper == "OPTIONS" ||
           upper == "TRACE";
}

bool IsSafePreTransferRetry(_In_ CURLcode code) {
    switch (code) {
...
std::chrono::milliseconds ParseRetryAfter(_In_ const HttpResponse& response) {
    ...
    try {
        ...
        if (consumed == value.size() && seconds >= 0) {
            const long long maxSeconds = static_cast<long long>(
                std::chrono::duration_cast<std::chrono::seconds>((std::chrono::milliseconds::max)()).count());
            const long long clamped = (std::min)(seconds, maxSeconds);
            return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::seconds(clamped));
        }
    } catch (...) {
    }
    ...
    if (retryAt != invalidTime) {
        ...
        if (now != invalidTime && retryAt > now) {
            const long double deltaSeconds = static_cast<long double>(retryAt) - static_cast<long double>(now);
            const long double maxSeconds = static_cast<long double>(
                std::chrono::duration_cast<std::chrono::seconds>((std::chrono::milliseconds::max)()).count());
            const long long clampedSeconds = static_cast<long long>((std::min)(deltaSeconds, maxSeconds));
            return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::seconds(clampedSeconds));
        }
    }
...
std::chrono::milliseconds ComputeRetryDelay(_In_ const RetryPolicy& policy, _In_ int retryIndex,
                                            _In_ const HttpResponse& response) {
...
bool SleepCancelable(_In_ std::chrono::milliseconds duration, _Inout_ std::atomic_bool* cancelFlag) {
    if (duration.count() <= 0) {
...
std::string CurlHttpVersionName(_In_ long version) {
    switch (version) {
...
std::optional<long> CurlProxyType(_In_ ProxyType type) {
    switch (type) {
    case ProxyType::Http:
        return static_cast<long>(CURLPROXY_HTTP);
#if LIBCURL_VERSION_NUM >= 0x073400
    case ProxyType::Https:
        return static_cast<long>(CURLPROXY_HTTPS);
#else
    case ProxyType::Https:
        return std::nullopt;
#endif
    case ProxyType::Socks4:
        return static_cast<long>(CURLPROXY_SOCKS4);
    case ProxyType::Socks4A:
        return static_cast<long>(CURLPROXY_SOCKS4A);
    case ProxyType::Socks5:
        return static_cast<long>(CURLPROXY_SOCKS5);
    case ProxyType::Socks5Hostname:
        return static_cast<long>(CURLPROXY_SOCKS5_HOSTNAME);
    }
    return std::nullopt;
}

long CurlHttpVersion(_In_ HttpVersion version) {
    switch (version) {
    case HttpVersion::Auto:
...
long CurlTlsVersion(_In_ TlsVersion version) {
    switch (version) {
    case TlsVersion::Default:
...
long CurlTlsMaxVersion(_In_ TlsVersion version) {
    switch (version) {
    case TlsVersion::Default:
...
long ComposeTlsVersionOption(_In_ TlsVersion minVersion, _In_ TlsVersion maxVersion) {
...
std::string RedactHeaderValue(const DebugOptions& debug, std::string_view name, std::string_view value);
std::string RedactUrlForDebug(const DebugOptions& debug, std::string_view input);

bool IsRedirectSensitiveHeaderName(_In_ const DebugOptions& debug, _In_ std::string_view name) {
    const std::string lower = ToLowerCopy(TrimCopy(name));
    static const std::array<std::string_view, 6> builtins{"x-api-key",      "api-key",  "x-auth-token",
                                                          "x-access-token", "x-secret", "set-cookie"};
    for (const auto item : builtins) {
        if (lower == item)
            return true;
    }
    for (const auto& item : debug.sensitiveHeaders) {
        if (lower == ToLowerCopy(TrimCopy(item)))
            return true;
    }
    return false;
}
...
struct CurlSlistHolder {
    ...
    bool Append(_In_ const std::string& line) {
...
struct TransferContext {
    ...
    void ResetForAttempt() {
        ...
        response->originalHeaders.clear();
        response->headers.Clear();
        response->date.reset();
        response->trailers.Clear();
        response->rawTrailers.clear();
        response->cookies.Clear();
        response->finalRawHeaders.clear();
...
std::size_t WriteCallback(_Out_ char* data, _In_ std::size_t size, _In_ std::size_t nmemb, _Inout_ void* userdata) {
    ...
    try {
        ...
        if (context->options->storeResponseBody && context->options->maxInMemoryResponseSize > 0) {
...
long ParseStatusCode(_In_ std::string_view statusLine) {
...
std::size_t HeaderCallback(_Out_ char* data, _In_ std::size_t size, _In_ std::size_t nmemb, _Inout_ void* userdata) {
    auto* context = static_cast<TransferContext*>(userdata);
    ...
    try {
        ...
        if (context->options && context->options->maxResponseHeaderSize > 0) {
            const std::uint64_t limit = static_cast<std::uint64_t>(context->options->maxResponseHeaderSize);
            if (context->receivedHeaderBytes > limit || chunkBytes > limit - context->receivedHeaderBytes) {
        ...
        context->response->originalHeaders.append(data, bytes);
...
std::size_t UploadReadCallback(_Out_ char* buffer, _In_ std::size_t size, _In_ std::size_t nmemb,
                               _Inout_ void* userdata) {
    auto* context = static_cast<TransferContext*>(userdata);
...
int UploadSeekCallback(_Inout_ void* userdata, _In_ curl_off_t offset, _In_ int origin) {
    auto* context = static_cast<TransferContext*>(userdata);
    if (!context || !context->options || !context->options->uploadSource) {
...
int ProgressCallbackBridge(_Inout_ void* userdata, _In_ curl_off_t dltotal, _In_ curl_off_t dlnow,
                           _In_ curl_off_t ultotal, _In_ curl_off_t ulnow) {
    auto* context = static_cast<TransferContext*>(userdata);
    ...
    try {
        if (context->options->cancelFlag && context->options->cancelFlag->load(std::memory_order_relaxed)) {
            context->callbackError = "请求已取消";
            context->callbackErrorCategory = HttpErrorCategory::Cancelled;
            return 1;
        }

        if (context->options->progressCallback && !context->options->progressCallback(dltotal, dlnow, ultotal, ulnow)) {
...
void EmitCurlVerbose(_In_ const DebugOptions& debug, _In_ std::string text) noexcept {
    try {
        if (debug.logger) {
            debug.logger(text);
        } else {
...
std::string RedactVerboseRequestLine(_In_ const DebugOptions& debug, _In_ std::string_view line) {
...
std::string RedactVerboseHeaderBlock(_In_ const DebugOptions& debug, _In_ const char* data, _In_ std::size_t size,
                                     _In_ bool outgoing) {
...
int CurlVerboseCallback(_Inout_ CURL*, _In_ curl_infotype type, _Out_ char* data, _In_ std::size_t size,
                        _Inout_ void* userdata) {
    auto* context = static_cast<TransferContext*>(userdata);
    if (!context || !context->options || !data || size == 0)
        return 0;
...
template <typename T>
bool SetCurlOption(_Inout_ CURL* curl, _In_ CURLoption option, _In_ const char* optionName, _In_ T value,
    ...
    response.success = false;
    response.curlCode = code;
    response.error = std::string("curl_easy_setopt(") + optionName + ") failed: " + curl_easy_strerror(code);
    return false;
}

bool ConfigureHeaders(_In_ const RequestOptions& options, _Inout_ CurlSlistHolder& headerList,
                      _Out_ HttpResponse& response) {
    for (const auto& item : options.headers.GetItems()) {
        if (!IsHttpToken(item.name) || !IsSafeHttpFieldValue(item.value)) {
            response.success = false;
            response.curlCode = CURLE_BAD_FUNCTION_ARGUMENT;
        ...
        if (!headerList.Append(line)) {
            response.success = false;
            response.curlCode = CURLE_OUT_OF_MEMORY;
...
bool ConfigureMime(_Inout_ CURL* curl, _In_ const RequestOptions& options, _Inout_ CurlMimeHolder& mime,
                   _Out_ HttpResponse& response) {
    ...
    for (const auto& part : options.multipart) {
        if (part.name.empty() || !IsSafeHttpFieldValue(part.name) || !IsSafeHttpFieldValue(part.fileName) ||
            !IsSafeHttpFieldValue(part.contentType) || ContainsNul(part.filePath)) {
            response.success = false;
            response.curlCode = CURLE_BAD_FUNCTION_ARGUMENT;
            response.errorCategory = HttpErrorCategory::InvalidArgument;
...
bool UsesCustomRequest(_In_ std::string_view method, _In_ std::string_view body, _In_ const RequestOptions& options) {
    const std::string upper = ToUpperCopy(method);
    if (!options.multipart.empty()) {
        return upper != "POST";
    }
...
[[maybe_unused]] bool HasRawHeaderNamed(_In_ const RequestOptions& options, _In_ std::string_view wantedName) {
...
bool HasRedirectSensitiveCustomHeader(_In_ const RequestOptions& options) {
    if (options.redirect.forwardSensitiveHeadersToOtherHosts)
        return false;
    for (const auto& item : options.headers.GetItems()) {
        if (IsRedirectSensitiveHeaderName(options.debug, item.name))
            return true;
    }
    for (const auto& raw : options.rawHeaderLines) {
        const std::size_t separator = raw.find_first_of(":;");
        if (IsRedirectSensitiveHeaderName(options.debug, std::string_view(raw).substr(0, separator))) {
            return true;
        }
    }
    return false;
...
bool NeedsLegacyAuthorizationRedirectGuard(_In_ const RequestOptions& options) {
#if LIBCURL_VERSION_NUM < 0x073A00
    return !options.redirect.forwardAuthToOtherHosts &&
           (options.headers.Contains("Authorization") || HasRawHeaderNamed(options, "Authorization"));
#else
    (void)options;
    return false;
#endif
...
bool NeedsLegacyCookieHeaderRedirectGuard(_In_ const RequestOptions& options) {
#if LIBCURL_VERSION_NUM < 0x074000
    return !options.redirect.forwardAuthToOtherHosts &&
           (options.headers.Contains("Cookie") || HasRawHeaderNamed(options, "Cookie"));
#else
    (void)options;
    return false;
#endif
...
bool NeedsLegacyCustomRedirectGuard(_In_ std::string_view method, _In_ std::string_view body,
                                    _In_ const RequestOptions& options) {
    return options.redirect.follow && UsesCustomRequest(method, body, options) &&
           !SupportsSafeCustomRedirectFollowing();
}

bool NeedsSensitiveRedirectGuard(_In_ std::string_view method, _In_ std::string_view body,
                                 _In_ const RequestOptions& options) {
    ...
    if (!options.tls.pinnedPublicKey.empty() && !options.redirect.allowUnpinnedRedirects)
        return true;
    if (!options.cookies.IsEmpty() && !options.redirect.allowExplicitCookiesOnRedirects)
        return true;
...
void ApplyRedirectSafetyRefusal(_Inout_ CURL* curl, _In_ std::string_view method, _In_ std::string_view body,
                                _In_ const RequestOptions& options, _Out_ HttpResponse& response) {
    if (!curl || response.curlCode != CURLE_OK || !NeedsSensitiveRedirectGuard(method, body, options)) {
        return;
    }
    ...
    if (infoCode != CURLE_OK) {
        response.success = false;
        response.curlCode = infoCode;
        response.errorCategory = HttpErrorCategory::Internal;
        response.error = std::string("读取安全重定向目标失败: ") + curl_easy_strerror(infoCode);
        return;
    }
    if (redirectUrl == nullptr || *redirectUrl == '\0')
        return;

    response.success = false;
    response.curlCode = CURLE_NOT_BUILT_IN;
    response.errorCategory = HttpErrorCategory::Internal;

    if (NeedsLegacyCustomRedirectGuard(method, body, options)) {
        response.error = "当前 libcurl 版本无法安全自动跟随使用 CURLOPT_CUSTOMREQUEST 的重定向；"
                         "已在发送下一跳之前停止。请升级到 libcurl 8.13.0 或更高版本，"
                         "或关闭 redirect.follow 并由业务层校验 Location 后重新发起请求";
        return;
    }
    if (!options.tls.pinnedPublicKey.empty() && !options.redirect.allowUnpinnedRedirects) {
        response.error = "检测到重定向，但当前请求配置了 pinnedPublicKey；为避免跳转到不受同一 Pin 保护的 Origin，"
                         "已在发送下一跳之前停止。可关闭 redirect.follow 手动校验 Location，"
                         "或在明确接受风险时设置 redirect.allowUnpinnedRedirects=true";
        return;
    }
    if (!options.cookies.IsEmpty() && !options.redirect.allowExplicitCookiesOnRedirects) {
        response.error = "检测到重定向，但 RequestOptions::cookies 通过 CURLOPT_COOKIE 发送，libcurl 会把显式 Cookie "
                         "继续用于后续重定向；"
    ...
    if (HasRedirectSensitiveCustomHeader(options)) {
        response.error = "检测到重定向且请求包含自定义敏感 Header；libcurl 无法保证跨主机跳转时自动剥离该 Header，"
                         "已在发送下一跳之前停止。可关闭 redirect.follow 手动校验 Location，"
                         "或在明确接受跨主机转发风险时设置 redirect.forwardSensitiveHeadersToOtherHosts=true";
        return;
    }
    if (NeedsLegacyAuthorizationRedirectGuard(options)) {
        response.error = "当前 libcurl 版本低于 7.58.0，无法安全自动保护跨主机重定向中的自定义 Authorization Header；"
                         "已在发送下一跳之前停止";
        return;
    }
    if (NeedsLegacyCookieHeaderRedirectGuard(options)) {
        response.error = "当前 libcurl 版本低于 7.64.0，无法安全自动保护跨主机重定向中的自定义 Cookie Header；"
                         "已在发送下一跳之前停止";
        return;
    }
}

bool ApplyMethod(_Inout_ CURL* curl, _In_ std::string_view method, _In_ std::string_view body,
                 _In_ const RequestOptions& options, _Inout_ TransferContext& context, _Inout_ CurlMimeHolder& mime,
                 _Out_ HttpResponse& response) {
    ...
    if (hasMultipart) {
        if (!SetCurlOption(curl, CURLOPT_MIMEPOST, "CURLOPT_MIMEPOST", mime.value, response)) {
            return false;
        }
        if (upper != "POST") {
            const std::string methodText(method);
            if (!SetCurlOption(curl, CURLOPT_CUSTOMREQUEST, "CURLOPT_CUSTOMREQUEST", methodText.c_str(), response)) {
                return false;
            }
        }
        return true;
    ...
    if (options.uploadSource) {
        if (!SetCurlOption(curl, CURLOPT_UPLOAD, "CURLOPT_UPLOAD", 1L, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_READFUNCTION, "CURLOPT_READFUNCTION", UploadReadCallback, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_READDATA, "CURLOPT_READDATA", &context, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_SEEKFUNCTION, "CURLOPT_SEEKFUNCTION", UploadSeekCallback, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_SEEKDATA, "CURLOPT_SEEKDATA", &context, response))
            return false;

        std::optional<curl_off_t> sizeValue = options.uploadSize;
        if (!sizeValue) {
            try {
                sizeValue = options.uploadSource->GetSize();
            } catch (const std::exception& ex) {
                response.success = false;
                response.curlCode = CURLE_READ_ERROR;
        ...
        if (sizeValue &&
            !SetCurlOption(curl, CURLOPT_INFILESIZE_LARGE, "CURLOPT_INFILESIZE_LARGE", *sizeValue, response)) {
            return false;
        }
    ...
    if (upper == "HEAD") {
        return SetCurlOption(curl, CURLOPT_NOBODY, "CURLOPT_NOBODY", 1L, response);
    }

    if (upper == "GET" && body.empty()) {
        return SetCurlOption(curl, CURLOPT_HTTPGET, "CURLOPT_HTTPGET", 1L, response);
    }

    if (upper == "POST") {
        if (!SetCurlOption(curl, CURLOPT_POST, "CURLOPT_POST", 1L, response)) {
            return false;
        }
        if (!SetCurlOption(curl, CURLOPT_POSTFIELDS, "CURLOPT_POSTFIELDS", body.empty() ? "" : body.data(), response)) {
            return false;
        }
        return SetCurlOption(curl, CURLOPT_POSTFIELDSIZE_LARGE, "CURLOPT_POSTFIELDSIZE_LARGE",
                             static_cast<curl_off_t>(body.size()), response);
    }
    ...
    if (!body.empty()) {
        if (!SetCurlOption(curl, CURLOPT_POSTFIELDS, "CURLOPT_POSTFIELDS", body.data(), response)) {
            return false;
        }
        if (!SetCurlOption(curl, CURLOPT_POSTFIELDSIZE_LARGE, "CURLOPT_POSTFIELDSIZE_LARGE",
                           static_cast<curl_off_t>(body.size()), response)) {
            return false;
        }
    }
...
bool ConfigureRequest(_Inout_ CURL* curl, _In_ std::string_view method, _In_ std::string_view url,
                      _In_ std::string_view body, _In_ const RequestOptions& options, _Inout_ TransferContext& context,
                      _Inout_ CurlSlistHolder& headerList, _Inout_ CurlSlistHolder& resolveList,
                      _Inout_ CurlMimeHolder& mime, _Inout_ std::array<char, CURL_ERROR_SIZE>& errorBuffer,
                      _Out_ HttpResponse& response) {
    ...
    if (!IsValidTlsVersion(options.tls.minVersion) || !IsValidTlsVersion(options.tls.maxVersion)) {
    ...
    if (options.retry.maxRetries < 0 || options.retry.baseDelay.count() < 0 || options.retry.maxDelay.count() < 0) {
    ...
    if (options.network.localPort > 0 && options.network.localPortRange > 65536L - options.network.localPort) {
    ...
    if (options.tls.minVersion != TlsVersion::Default && options.tls.maxVersion != TlsVersion::Default &&
    ...
    if (rejectNul(options.username, "username") || rejectNul(options.password, "password") ||
        ...
        rejectNul(options.tls.clientCertificateType, "clientCertificateType") ||
        rejectNul(options.tls.clientKey, "clientKey") || rejectNul(options.tls.clientKeyType, "clientKeyType") ||
        rejectNul(options.tls.clientKeyPassword, "clientKeyPassword") ||
        rejectNul(options.tls.pinnedPublicKey, "pinnedPublicKey") || rejectNul(options.tls.cipherList, "cipherList") ||
        rejectNul(options.tls.tls13CipherList, "tls13CipherList") || rejectNul(options.tls.crlFile, "crlFile"))
        return false;
    if (options.proxy &&
        (rejectNul(options.proxy->url, "proxy.url") || rejectNul(options.proxy->username, "proxy.username") ||
         rejectNul(options.proxy->password, "proxy.password") || rejectNul(options.proxy->noProxy, "proxy.noProxy")))
        return false;
    ...
    for (const auto& entry : options.network.resolve) {
        if (rejectNul(entry.host, "resolve.host") || rejectNul(entry.address, "resolve.address"))
            return false;
        if (!IsSafeHttpFieldValue(entry.host) || !IsSafeHttpFieldValue(entry.address) || ContainsCrLf(entry.host) ||
            ContainsCrLf(entry.address)) {
    ...
    if ((!options.bearerToken.empty() || !options.username.empty()) &&
        (options.headers.Contains("Authorization") || hasRawHeader("Authorization"))) {
    ...
    if (options.proxy && !options.proxy->url.empty() &&
        ...
        (options.headers.Contains("Proxy-Authorization") || hasRawHeader("Proxy-Authorization"))) {
    ...
    for (const auto& cookie : options.cookies.GetAllCookies(false)) {
        if (!IsHttpToken(cookie.name) || !IsSafeHttpFieldValue(cookie.value) ||
            cookie.value.find(';') != std::string::npos) {
    ...
    if (!SetCurlOption(curl, CURLOPT_URL, "CURLOPT_URL", urlText.c_str(), response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_ERRORBUFFER, "CURLOPT_ERRORBUFFER", errorBuffer.data(), response))
        return false;
    // 本封装同时提供同步客户端和多线程 curl_multi 客户端，统一禁用 libcurl 的信号路径，
    // 避免多线程环境中信号处理器恢复产生竞态。
    if (!SetCurlOption(curl, CURLOPT_NOSIGNAL, "CURLOPT_NOSIGNAL", 1L, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_WRITEFUNCTION, "CURLOPT_WRITEFUNCTION", WriteCallback, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_WRITEDATA, "CURLOPT_WRITEDATA", &context, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_HEADERFUNCTION, "CURLOPT_HEADERFUNCTION", HeaderCallback, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_HEADERDATA, "CURLOPT_HEADERDATA", &context, response))
        return false;

    if (headerList.value &&
        !SetCurlOption(curl, CURLOPT_HTTPHEADER, "CURLOPT_HTTPHEADER", headerList.value, response)) {
        return false;
    }

    // HTTPS 经 HTTP 代理时不把 CONNECT 隧道 Header 混入目标服务器 Header。
    if (!SetCurlOption(curl, CURLOPT_SUPPRESS_CONNECT_HEADERS, "CURLOPT_SUPPRESS_CONNECT_HEADERS", 1L, response))
        return false;

    const bool redirectSafetyGuard = NeedsSensitiveRedirectGuard(method, body, options);
#if LIBCURL_VERSION_NUM >= 0x080D00
    ...
    const long followLocationMode =
        options.redirect.follow ? (redirectSafetyGuard ? 0L : static_cast<long>(CURLFOLLOW_OBEYCODE)) : 0L;
#else
    // 旧版 libcurl 的 FOLLOWLOCATION 会让 CUSTOMREQUEST 覆盖后续重定向 Method。
    // 对这类请求先不自动跟随；完成首跳后若存在重定向，再返回明确的 fail-closed 错误。
    const long followLocationMode = (options.redirect.follow && !redirectSafetyGuard) ? 1L : 0L;
#endif
    if (!SetCurlOption(curl, CURLOPT_FOLLOWLOCATION, "CURLOPT_FOLLOWLOCATION", followLocationMode, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_MAXREDIRS, "CURLOPT_MAXREDIRS", options.redirect.maxRedirects, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_AUTOREFERER, "CURLOPT_AUTOREFERER", options.redirect.autoReferer ? 1L : 0L,
                       response))
        return false;

    if (options.autoDecompress) {
        const char* encoding = options.acceptEncoding.empty() ? "" : options.acceptEncoding.c_str();
        if (!SetCurlOption(curl, CURLOPT_ACCEPT_ENCODING, "CURLOPT_ACCEPT_ENCODING", encoding, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_HTTP_CONTENT_DECODING, "CURLOPT_HTTP_CONTENT_DECODING", 1L, response))
            return false;
    } else {
        if (!options.acceptEncoding.empty() && !SetCurlOption(curl, CURLOPT_ACCEPT_ENCODING, "CURLOPT_ACCEPT_ENCODING",
                                                              options.acceptEncoding.c_str(), response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_HTTP_CONTENT_DECODING, "CURLOPT_HTTP_CONTENT_DECODING", 0L, response))
            return false;
    }

    if (!SetCurlOption(curl, CURLOPT_CONNECTTIMEOUT_MS, "CURLOPT_CONNECTTIMEOUT_MS",
                       static_cast<long>((std::min)(options.connectTimeout.count(),
                                                    static_cast<std::int64_t>((std::numeric_limits<long>::max)()))),
                       response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_TIMEOUT_MS, "CURLOPT_TIMEOUT_MS",
                       static_cast<long>((std::min)(options.timeout.count(),
                                                    static_cast<std::int64_t>((std::numeric_limits<long>::max)()))),
                       response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_LOW_SPEED_LIMIT, "CURLOPT_LOW_SPEED_LIMIT", options.lowSpeedLimitBytesPerSecond,
                       response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_LOW_SPEED_TIME, "CURLOPT_LOW_SPEED_TIME",
                       static_cast<long>(
                           (std::min)(options.lowSpeedTime.count(), static_cast<decltype(options.lowSpeedTime.count())>(
                                                                        (std::numeric_limits<long>::max)()))),
                       response))
        return false;

    if (!SetCurlOption(curl, CURLOPT_SSL_VERIFYPEER, "CURLOPT_SSL_VERIFYPEER", options.tls.verifyPeer ? 1L : 0L,
                       response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_SSL_VERIFYHOST, "CURLOPT_SSL_VERIFYHOST", options.tls.verifyHost ? 2L : 0L,
                       response))
        return false;
    ...
    if (sslOptions != 0L && !SetCurlOption(curl, CURLOPT_SSL_OPTIONS, "CURLOPT_SSL_OPTIONS", sslOptions, response)) {
        return false;
    }

    // HTTPS 代理自身也需要 TLS；沿用同一个 SSL_OPTIONS bitmask，避免目标 TLS 与代理 TLS
    // 在 Native CA / Schannel 吊销策略上出现不一致。
    if (options.proxy && !options.proxy->url.empty() && IsHttpsProxy(*options.proxy) && sslOptions != 0L &&
        !SetCurlOption(curl, CURLOPT_PROXY_SSL_OPTIONS, "CURLOPT_PROXY_SSL_OPTIONS", sslOptions, response)) {
        return false;
    }
    ...
    if (options.tls.verifyStatus &&
        !SetCurlOption(curl, CURLOPT_SSL_VERIFYSTATUS, "CURLOPT_SSL_VERIFYSTATUS", 1L, response))
        return false;
#endif

    if (!options.tls.caFile.empty() &&
        !SetCurlOption(curl, CURLOPT_CAINFO, "CURLOPT_CAINFO", options.tls.caFile.c_str(), response))
        return false;
    if (!options.tls.caPath.empty() &&
        !SetCurlOption(curl, CURLOPT_CAPATH, "CURLOPT_CAPATH", options.tls.caPath.c_str(), response))
        return false;

    LogTlsConfigurationDebug(curl, options);

    if (!options.tls.clientCertificate.empty() &&
        !SetCurlOption(curl, CURLOPT_SSLCERT, "CURLOPT_SSLCERT", options.tls.clientCertificate.c_str(), response))
        return false;
    if (!options.tls.clientCertificateType.empty() &&
        !SetCurlOption(curl, CURLOPT_SSLCERTTYPE, "CURLOPT_SSLCERTTYPE", options.tls.clientCertificateType.c_str(),
                       response))
        return false;
    if (!options.tls.clientKey.empty() &&
        !SetCurlOption(curl, CURLOPT_SSLKEY, "CURLOPT_SSLKEY", options.tls.clientKey.c_str(), response))
        return false;
    if (!options.tls.clientKeyType.empty() &&
        !SetCurlOption(curl, CURLOPT_SSLKEYTYPE, "CURLOPT_SSLKEYTYPE", options.tls.clientKeyType.c_str(), response))
        return false;
    if (!options.tls.clientKeyPassword.empty() &&
        !SetCurlOption(curl, CURLOPT_KEYPASSWD, "CURLOPT_KEYPASSWD", options.tls.clientKeyPassword.c_str(), response))
        return false;
    if (!options.tls.pinnedPublicKey.empty() && !SetCurlOption(curl, CURLOPT_PINNEDPUBLICKEY, "CURLOPT_PINNEDPUBLICKEY",
                                                               options.tls.pinnedPublicKey.c_str(), response))
        return false;
    if (!options.tls.cipherList.empty() && !SetCurlOption(curl, CURLOPT_SSL_CIPHER_LIST, "CURLOPT_SSL_CIPHER_LIST",
                                                          options.tls.cipherList.c_str(), response))
        return false;
#if LIBCURL_VERSION_NUM >= 0x073D00
    if (!options.tls.tls13CipherList.empty() && !SetCurlOption(curl, CURLOPT_TLS13_CIPHERS, "CURLOPT_TLS13_CIPHERS",
                                                               options.tls.tls13CipherList.c_str(), response))
        return false;
#else
    if (!options.tls.tls13CipherList.empty()) {
        response.curlCode = CURLE_NOT_BUILT_IN;
        response.error = "当前 libcurl 版本不支持 TLS 1.3 Cipher 配置";
        return false;
    }
#endif
    if (!options.tls.crlFile.empty() &&
        !SetCurlOption(curl, CURLOPT_CRLFILE, "CURLOPT_CRLFILE", options.tls.crlFile.c_str(), response))
        return false;
    ...
    if (options.tls.minVersion != TlsVersion::Default || options.tls.maxVersion != TlsVersion::Default) {
        ...
        if (!SetCurlOption(curl, CURLOPT_SSLVERSION, "CURLOPT_SSLVERSION", tlsVersion, response))
            return false;
        if (options.proxy && !options.proxy->url.empty() && IsHttpsProxy(*options.proxy) &&
            !SetCurlOption(curl, CURLOPT_PROXY_SSLVERSION, "CURLOPT_PROXY_SSLVERSION", tlsVersion, response))
            return false;
    }

    // Auto 表示完全采用当前 libcurl 的默认 HTTP 协商策略。
    if (options.httpVersion != HttpVersion::Auto && !SetCurlOption(curl, CURLOPT_HTTP_VERSION, "CURLOPT_HTTP_VERSION",
                                                                   CurlHttpVersion(options.httpVersion), response))
        return false;

    // 本封装默认仅允许 HTTP/HTTPS，避免用户可控 URL 或重定向访问 FILE、FTP、SMB 等协议。
#if LIBCURL_VERSION_NUM >= 0x075500
    if (!SetCurlOption(curl, CURLOPT_PROTOCOLS_STR, "CURLOPT_PROTOCOLS_STR", "http,https", response))
        return false;
    {
        ...
        if (!SetCurlOption(curl, CURLOPT_REDIR_PROTOCOLS_STR, "CURLOPT_REDIR_PROTOCOLS_STR", redirectProtocols,
                           response))
            return false;
    }
#else
    if (!SetCurlOption(curl, CURLOPT_PROTOCOLS, "CURLOPT_PROTOCOLS",
                       static_cast<long>(CURLPROTO_HTTP | CURLPROTO_HTTPS), response))
        return false;
    {
        ...
        if (!SetCurlOption(curl, CURLOPT_REDIR_PROTOCOLS, "CURLOPT_REDIR_PROTOCOLS", redirectProtocols, response))
            return false;
    }
#endif

#if LIBCURL_VERSION_NUM >= 0x073D00
    if (!SetCurlOption(curl, CURLOPT_DISALLOW_USERNAME_IN_URL, "CURLOPT_DISALLOW_USERNAME_IN_URL",
                       options.disallowUsernameInUrl ? 1L : 0L, response))
        return false;
#else
    ...
    if (!SetCurlOption(curl, CURLOPT_FAILONERROR, "CURLOPT_FAILONERROR", options.failOnHttpError ? 1L : 0L, response))
        return false;
    if (!SetCurlOption(curl, CURLOPT_UNRESTRICTED_AUTH, "CURLOPT_UNRESTRICTED_AUTH",
                       options.redirect.forwardAuthToOtherHosts ? 1L : 0L, response))
        return false;

    if (!SetCurlOption(curl, CURLOPT_TCP_KEEPALIVE, "CURLOPT_TCP_KEEPALIVE", options.tcpKeepAlive ? 1L : 0L, response))
        return false;
    if (options.tcpKeepAlive) {
        if (!SetCurlOption(curl, CURLOPT_TCP_KEEPIDLE, "CURLOPT_TCP_KEEPIDLE", options.tcpKeepIdleSeconds, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_TCP_KEEPINTVL, "CURLOPT_TCP_KEEPINTVL", options.tcpKeepIntervalSeconds,
                           response))
            return false;
    }
    ...
    if (options.maxResponseSize > 0) {
        const auto maxCurl =
            static_cast<curl_off_t>((std::min)(static_cast<std::uint64_t>(options.maxResponseSize),
                                               static_cast<std::uint64_t>((std::numeric_limits<curl_off_t>::max)())));
        if (!SetCurlOption(curl, CURLOPT_MAXFILESIZE_LARGE, "CURLOPT_MAXFILESIZE_LARGE", maxCurl, response))
            return false;
    }

    if (!options.userAgent.empty() &&
        !SetCurlOption(curl, CURLOPT_USERAGENT, "CURLOPT_USERAGENT", options.userAgent.c_str(), response))
        return false;

    if (options.enableCookieEngine) {
        const char* cookieFile = options.cookieFile.empty() ? "" : options.cookieFile.c_str();
        if (!SetCurlOption(curl, CURLOPT_COOKIEFILE, "CURLOPT_COOKIEFILE", cookieFile, response))
            return false;
        if (options.newCookieSession &&
            !SetCurlOption(curl, CURLOPT_COOKIESESSION, "CURLOPT_COOKIESESSION", 1L, response))
            return false;
    } else {
        // 显式关闭 Cookie Engine，避免复用 easy handle 时仅清空 Cookie 数据但引擎仍继续接收 Set-Cookie。
        const char* noCookieFile = nullptr;
        if (!SetCurlOption(curl, CURLOPT_COOKIEFILE, "CURLOPT_COOKIEFILE", noCookieFile, response))
            return false;
    }

    if (!options.cookieJar.empty() &&
        !SetCurlOption(curl, CURLOPT_COOKIEJAR, "CURLOPT_COOKIEJAR", options.cookieJar.c_str(), response))
        return false;
    ...
    if (!explicitCookies.empty() &&
        !SetCurlOption(curl, CURLOPT_COOKIE, "CURLOPT_COOKIE", explicitCookies.c_str(), response))
        return false;

    if (options.proxy) {
        // optional 已存在表示调用方明确接管代理策略；空字符串按 libcurl 语义显式禁用环境代理。
        if (!SetCurlOption(curl, CURLOPT_PROXY, "CURLOPT_PROXY", options.proxy->url.c_str(), response))
            return false;
        if (!options.proxy->url.empty() && options.proxy->type) {
            ...
            if (!SetCurlOption(curl, CURLOPT_PROXYTYPE, "CURLOPT_PROXYTYPE", *proxyType, response))
                return false;
        }
        if (!options.proxy->url.empty()) {
            if (!options.proxy->username.empty() && !SetCurlOption(curl, CURLOPT_PROXYUSERNAME, "CURLOPT_PROXYUSERNAME",
                                                                   options.proxy->username.c_str(), response))
                return false;
            if (!options.proxy->password.empty() && !SetCurlOption(curl, CURLOPT_PROXYPASSWORD, "CURLOPT_PROXYPASSWORD",
                                                                   options.proxy->password.c_str(), response))
                return false;
            if (!SetCurlOption(curl, CURLOPT_PROXYAUTH, "CURLOPT_PROXYAUTH", static_cast<long>(options.proxy->auth),
                               response))
                return false;
        }
        if (!options.proxy->noProxy.empty() &&
            !SetCurlOption(curl, CURLOPT_NOPROXY, "CURLOPT_NOPROXY", options.proxy->noProxy.c_str(), response))
            return false;
    }

    if (!options.bearerToken.empty()) {
#if LIBCURL_VERSION_NUM >= 0x073D00
        if (!SetCurlOption(curl, CURLOPT_HTTPAUTH, "CURLOPT_HTTPAUTH", static_cast<long>(CURLAUTH_BEARER), response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_XOAUTH2_BEARER, "CURLOPT_XOAUTH2_BEARER", options.bearerToken.c_str(),
                           response))
            return false;
#else
        response.success = false;
        ...
        if (!SetCurlOption(curl, CURLOPT_USERNAME, "CURLOPT_USERNAME", options.username.c_str(), response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_PASSWORD, "CURLOPT_PASSWORD", options.password.c_str(), response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_HTTPAUTH, "CURLOPT_HTTPAUTH", static_cast<long>(options.httpAuth), response))
            return false;
    }

    if (options.cancelFlag || options.progressCallback) {
        if (!SetCurlOption(curl, CURLOPT_NOPROGRESS, "CURLOPT_NOPROGRESS", 0L, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_XFERINFOFUNCTION, "CURLOPT_XFERINFOFUNCTION", ProgressCallbackBridge,
                           response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_XFERINFODATA, "CURLOPT_XFERINFODATA", &context, response))
            return false;
    } else {
        if (!SetCurlOption(curl, CURLOPT_NOPROGRESS, "CURLOPT_NOPROGRESS", 1L, response))
            return false;
    }

    if (!options.network.interfaceName.empty() &&
        !SetCurlOption(curl, CURLOPT_INTERFACE, "CURLOPT_INTERFACE", options.network.interfaceName.c_str(), response))
        return false;
    if (options.network.localPort > 0 &&
        !SetCurlOption(curl, CURLOPT_LOCALPORT, "CURLOPT_LOCALPORT", options.network.localPort, response))
        return false;
    if (options.network.localPortRange > 1 && !SetCurlOption(curl, CURLOPT_LOCALPORTRANGE, "CURLOPT_LOCALPORTRANGE",
                                                             options.network.localPortRange, response))
        return false;
    for (const auto& entry : options.network.resolve) {
        if (entry.host.empty() || entry.port == 0 || entry.address.empty())
            continue;
        ...
        if (address.find(':') != std::string::npos &&
            !(address.size() >= 2 && address.front() == '[' && address.back() == ']')) {
            address = "[" + address + "]";
        }
    ...
    if (resolveList.value && !SetCurlOption(curl, CURLOPT_RESOLVE, "CURLOPT_RESOLVE", resolveList.value, response))
        return false;

    if (options.speed.maxDownloadBytesPerSecond > 0 &&
        !SetCurlOption(curl, CURLOPT_MAX_RECV_SPEED_LARGE, "CURLOPT_MAX_RECV_SPEED_LARGE",
                       options.speed.maxDownloadBytesPerSecond, response))
        return false;
    if (options.speed.maxUploadBytesPerSecond > 0 &&
        !SetCurlOption(curl, CURLOPT_MAX_SEND_SPEED_LARGE, "CURLOPT_MAX_SEND_SPEED_LARGE",
                       options.speed.maxUploadBytesPerSecond, response))
        return false;
    if (!options.range.empty() && !SetCurlOption(curl, CURLOPT_RANGE, "CURLOPT_RANGE", options.range.c_str(), response))
        return false;
    if (options.resumeFrom &&
        !SetCurlOption(curl, CURLOPT_RESUME_FROM_LARGE, "CURLOPT_RESUME_FROM_LARGE", *options.resumeFrom, response))
        return false;
    if (options.debug.enabled && options.debug.curlVerbose) {
        ...
        if (!SetCurlOption(curl, CURLOPT_DEBUGFUNCTION, "CURLOPT_DEBUGFUNCTION", CurlVerboseCallback, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_DEBUGDATA, "CURLOPT_DEBUGDATA", &context, response))
            return false;
        if (!SetCurlOption(curl, CURLOPT_VERBOSE, "CURLOPT_VERBOSE", 1L, response))
            return false;
    }
...
void FinalizeHeaderResult(_Inout_ TransferContext& context, _Out_ HttpResponse& response) {
    context.FinalizeCurrentBlock();
    if (!response.headerHistory.empty()) {
        const auto& finalBlock = response.headerHistory.back();
        response.headers = finalBlock.headers;
        response.finalRawHeaders = finalBlock.rawHeaders;
        ...
        response.cookies.ParseFromSetCookieHeaders(finalBlock.headers.GetHeaderValues("Set-Cookie"));
        for (const auto& trailer : response.trailers.GetItems()) {
            response.headers.AddHeader(trailer.name, trailer.value);
        }
    }
}

void CollectInfo(_Inout_ CURL* curl, _Out_ HttpResponse& response) {
    long code = 0;
...
bool ShouldRetry(_In_ std::string_view method, _In_ const RetryPolicy& policy, _Out_ HttpResponse& response,
                 _In_ int retryIndex) {
    ...
    if (policy.customShouldRetry) {
        try {
            const auto decision = policy.customShouldRetry(response.curlCode, response.code, retryIndex);
    ...
    if (response.curlCode != CURLE_OK) {
        ...
        // 这里仍允许基于状态码的重试策略继续判断。
        if (response.curlCode == CURLE_HTTP_RETURNED_ERROR &&
            ContainsStatus(policy.retryHttpStatusCodes, response.code)) {
            return idempotent || policy.retryNonIdempotent;
        }

        if (!ContainsCode(policy.retryCurlCodes, response.curlCode)) {
            return false;
        }
        if (idempotent || policy.retryNonIdempotent) {
            return true;
        }
        return IsSafePreTransferRetry(response.curlCode);
    }
...
bool PrepareExternalResponseSinkForRetry(_In_ const RequestOptions& options, _In_ const TransferContext& context,
                                         _Out_ HttpResponse& response) {
    const bool hasExternalSink = static_cast<bool>(options.responseChunkCallback) || options.responseStream != nullptr;
...
bool PrepareExternalSinkForLogicalReplay(_In_ const RequestOptions& options, _Out_ HttpResponse& response) {
    const bool hasExternalSink = static_cast<bool>(options.responseChunkCallback) || options.responseStream != nullptr;
    if (!hasExternalSink)
        return true;
    if (!options.responseRetryResetCallback) {
        if (!response.error.empty())
            response.error += "; ";
        response.error +=
            "请求需要认证重放，但配置了外部响应接收器且没有 responseRetryResetCallback，为避免重复数据已取消自动重放";
        return false;
    }
    try {
        if (options.responseRetryResetCallback())
...
bool RewindUploadForLogicalReplay(_In_ const RequestOptions& options, _Out_ HttpResponse& response) {
    if (!options.uploadSource)
        return true;
    try {
    ...
    response.success = false;
    response.curlCode = CURLE_SEND_FAIL_REWIND;
    response.errorCategory = HttpErrorCategory::Upload;
    return false;
}

HttpResponse PerformOnHandle(_Inout_ CURL* curl, _In_ std::string_view method, _In_ std::string_view url,
    ...
    if (!ConfigureRequest(curl, method, url, body, options, context, headerList, resolveList, mime, errorBuffer,
                          response)) {
        return response;
    }
...
HttpResponse DownloadWithInvoker(_In_ const std::string& filePath, _In_ RequestOptions options,
                                 _In_ const std::function<HttpResponse(const RequestOptions&)>& invoker) {
    ...
    if (resumeOffset > 0) {
        ...
        if (existingSize < 0 ||
            static_cast<std::uintmax_t>(existingSize) != static_cast<std::uintmax_t>(resumeOffset)) {
            HttpResponse response;
            response.curlCode = CURLE_BAD_FUNCTION_ARGUMENT;
    ...
    const std::string workingPath = resumeMode ? filePath : filePath + ".curl_ex_tmp_" + makeFileSuffix();

    std::ofstream file;
    const auto openFile = [&]() -> bool {
        file.close();
        file.clear();
        file.open(workingPath, resumeMode ? (std::ios::binary | std::ios::app) : (std::ios::binary | std::ios::trunc));
        return static_cast<bool>(file);
    };
    if (!openFile()) {
        HttpResponse response;
        response.curlCode = CURLE_WRITE_ERROR;
    ...
    options.responseRetryResetCallback = [&file, &callbackBytes, workingPath, resumeOffset, userResetCallback,
                                          hasUserExternalSink, resumeMode]() {
        try {
            file.flush();
            file.close();
            const std::uintmax_t rollbackSize =
                resumeMode ? static_cast<std::uintmax_t>(resumeOffset) : std::uintmax_t{0};
            std::filesystem::resize_file(workingPath, rollbackSize);
            file.clear();
            file.open(workingPath,
                      resumeMode ? (std::ios::binary | std::ios::app) : (std::ios::binary | std::ios::trunc));
            callbackBytes = 0;
            if (!file)
                return false;
            if (hasUserExternalSink && !userResetCallback)
                return false;
    ...
    if (!fileOk && response.IsTransportSuccessful()) {
        response.success = false;
        response.curlCode = CURLE_WRITE_ERROR;
    ...
    if (!response.IsSuccessful() || !fileOk) {
        std::error_code ec;
        if (resumeMode) {
    ...
    if (targetExists) {
        ...
        committed = ::ReplaceFileW(targetPath.c_str(), sourcePath.c_str(), nullptr, 0, nullptr, nullptr);
    } else {
        committed = ::MoveFileExW(sourcePath.c_str(), targetPath.c_str(), MOVEFILE_WRITE_THROUGH);
    }
    if (!committed) {
        ...
        response.errorCategory = HttpErrorCategory::Download;
        response.error =
            "下载完成但 Windows 原子提交目标文件失败，错误码=" + std::to_string(static_cast<unsigned long>(winError));
        return response;
    }
    return response;
#else
...
CURLcode Initialize() {
    return GetGlobalRuntime().GetCode();
}
...
std::string UrlEncode(_In_ std::string_view input) {
    ...
    char* output = curl_easy_escape(curl, input.data(), static_cast<int>(input.size()));
    std::string result;
    if (output) {
        result = output;
        curl_free(output);
...
std::string UrlDecode(_In_ std::string_view input) {
    ...
    int outputLength = 0;
    char* output = curl_easy_unescape(curl, input.data(), static_cast<int>(input.size()), &outputLength);
    std::string result;
    if (output) {
...
UrlParameters::UrlParameters(_In_ std::string_view query) {
    Parse(query);
}

void UrlParameters::Parse(_In_ std::string_view query) {
    params_.clear();
...
std::string UrlParameters::ToString(_In_ bool leadingQuestionMark) const {
    std::ostringstream out;
...
std::string UrlParameters::Get(_In_ const std::string& key) const {
    const auto it =
        std::find_if(params_.begin(), params_.end(), [&](const UrlParamItem& item) { return item.key == key; });
    return it == params_.end() ? std::string{} : it->value;
}

std::vector<std::string> UrlParameters::GetAll(_In_ const std::string& key) const {
...
void UrlParameters::Set(_In_ const std::string& key, _In_ const std::string& value) {
    Remove(key);
    Add(key, value, true);
}

void UrlParameters::Add(_In_ const std::string& key, _In_ const std::string& value, _In_ bool hasEquals) {
    params_.push_back(UrlParamItem{key, value, hasEquals});
}

void UrlParameters::Remove(_In_ const std::string& key) {
    params_.erase(
        std::remove_if(params_.begin(), params_.end(), [&](const UrlParamItem& item) { return item.key == key; }),
        params_.end());
}

bool UrlParameters::Has(_In_ const std::string& key) const {
    return std::any_of(params_.begin(), params_.end(), [&](const UrlParamItem& item) { return item.key == key; });
}

std::size_t UrlParameters::GetSize() const noexcept {
    return params_.size();
}

bool UrlParameters::IsEmpty() const noexcept {
    return params_.empty();
}

void UrlParameters::Clear() noexcept {
    params_.clear();
}

std::vector<std::string> UrlParameters::GetAllParameters() const {
...
std::vector<std::string> UrlParameters::GetAllParameterNames() const {
...
std::string& UrlParameters::operator[](const std::string& key) {
    auto it = std::find_if(params_.begin(), params_.end(), [&](const UrlParamItem& item) { return item.key == key; });
    if (it == params_.end()) {
...
HttpHeaders::HttpHeaders(_In_ std::string_view rawHeaders) {
    ParseHeaders(rawHeaders);
}

std::string HttpHeaders::Trim(_In_ std::string_view str) {
    return TrimCopy(str);
}

bool HttpHeaders::EqualsIgnoreCase(_In_ std::string_view a, _In_ std::string_view b) noexcept {
    if (a.size() != b.size()) {
        return false;
    }
...
bool HttpHeaders::IsSafeHeaderName(_In_ std::string_view value) noexcept {
    return IsHttpToken(value);
}

bool HttpHeaders::IsSafeHeaderValue(_In_ std::string_view value) noexcept {
    return IsSafeHttpFieldValue(value);
}

void HttpHeaders::ParseHeaders(_In_ std::string_view rawHeaders) {
    headers_.clear();
    std::size_t start = 0;
...
bool HttpHeaders::AddHeader(_In_ const std::string& key, _In_ const std::string& value) {
...
bool HttpHeaders::SetHeader(_In_ const std::string& key, _In_ const std::string& value) {
    const std::string name = Trim(key);
    const std::string val = Trim(value);
    if (!IsSafeHeaderName(name) || !IsSafeHeaderValue(val)) {
        return false;
...
bool HttpHeaders::SetDefaultHeader(_In_ const std::string& key, _In_ const std::string& value) {
    if (Contains(key)) {
        return false;
    }
...
bool HttpHeaders::AppendHeader(_In_ const std::string& key, _In_ const std::string& value,
    ...
    auto it = std::find_if(headers_.begin(), headers_.end(),
                           [&](const HttpHeaderItem& item) { return EqualsIgnoreCase(item.name, name); });
    if (it == headers_.end()) {
        headers_.push_back({name, val});
    } else {
        it->value += delimiter;
...
bool HttpHeaders::EraseHeader(_In_ const std::string& key) {
    const std::string name = Trim(key);
...
bool HttpHeaders::Contains(_In_ const std::string& key) const {
    const std::string name = Trim(key);
    return std::any_of(headers_.begin(), headers_.end(),
                       [&](const HttpHeaderItem& item) { return EqualsIgnoreCase(item.name, name); });
}

std::string HttpHeaders::GetHeaderValue(_In_ const std::string& key) const {
    const std::string name = Trim(key);
    const auto it = std::find_if(headers_.begin(), headers_.end(),
                                 [&](const HttpHeaderItem& item) { return EqualsIgnoreCase(item.name, name); });
    return it == headers_.end() ? std::string{} : it->value;
}

std::vector<std::string> HttpHeaders::GetHeaderValues(_In_ const std::string& key) const {
    const std::string name = Trim(key);
    std::vector<std::string> values;
...
std::vector<std::string> HttpHeaders::GetKeys(_In_ bool unique) const {
    ...
    for (const auto& item : headers_) {
        ...
        const bool alreadyPresent = std::any_of(
            keys.begin(), keys.end(), [&](const std::string& key) { return EqualsIgnoreCase(key, item.name); });
        if (!alreadyPresent) {
            keys.push_back(item.name);
        }
    }
...
HttpCookies::HttpCookies(_In_ const std::vector<std::string>& setCookieHeaders) {
    ParseFromSetCookieHeaders(setCookieHeaders);
}

HttpCookies::HttpCookies(_In_ std::string_view requestCookieString) {
    ParseFromCookieString(requestCookieString);
}

std::string HttpCookies::Trim(_In_ std::string_view str) {
    return TrimCopy(str);
}

bool HttpCookies::HasSameIdentity(_In_ const Cookie& a, _In_ const Cookie& b) {
    ...
    return a.name == b.name && normalizeDomain(a.domain) == normalizeDomain(b.domain) && a.path == b.path;
}

void HttpCookies::Merge(_In_ const HttpCookies& other, _In_ bool overwrite) {
    for (const auto& cookie : other.cookies_) {
        auto it = std::find_if(cookies_.begin(), cookies_.end(),
                               [&](const Cookie& existing) { return HasSameIdentity(existing, cookie); });
        if (it == cookies_.end()) {
...
HttpCookies HttpCookies::MergedWith(_In_ const HttpCookies& other, _In_ bool overwrite) const {
    HttpCookies result = *this;
    result.Merge(other, overwrite);
    return result;
}

void HttpCookies::ParseFromSetCookieHeaders(_In_ const std::vector<std::string>& setCookieHeaders) {
    cookies_.clear();
        ...
        while (start <= header.size()) {
            ...
            if (!segment.empty()) {
                ...
                const std::string value =
                    equal == std::string::npos ? std::string{} : Trim(std::string_view(segment).substr(equal + 1));
...
void HttpCookies::ParseFromCookieString(_In_ std::string_view cookieString) {
    cookies_.clear();
    std::size_t start = 0;
...
std::vector<std::string> HttpCookies::ExtractSetCookieHeaders(_In_ std::string_view rawHeaders) {
    HttpHeaders parsed(rawHeaders);
    return parsed.GetHeaderValues("Set-Cookie");
}

void HttpCookies::SetCookie(_In_ const Cookie& cookie) {
    if (cookie.name.empty()) {
        return;
    }
    auto it = std::find_if(cookies_.begin(), cookies_.end(),
...
void HttpCookies::SetCookie(_In_ const std::string& name, _In_ const std::string& value) {
...
bool HttpCookies::EraseCookie(_In_ const std::string& name) {
    const auto oldSize = cookies_.size();
    cookies_.erase(std::remove_if(cookies_.begin(), cookies_.end(),
                                  [&](const Cookie& cookie) { return cookie.name == Trim(name); }),
                   cookies_.end());
    return cookies_.size() != oldSize;
}

bool HttpCookies::EraseCookie(_In_ const std::string& name, _In_ const std::optional<std::string>& domain,
    ...
    const auto oldSize = cookies_.size();
    cookies_.erase(std::remove_if(cookies_.begin(), cookies_.end(),
                                  [&](const Cookie& cookie) { return HasSameIdentity(cookie, identity); }),
                   cookies_.end());
    return cookies_.size() != oldSize;
}

bool HttpCookies::Contains(_In_ const std::string& name) const {
    const std::string key = Trim(name);
    return std::any_of(cookies_.begin(), cookies_.end(), [&](const Cookie& cookie) { return cookie.name == key; });
}

void HttpCookies::RemoveEmptyCookies() {
    cookies_.erase(
        std::remove_if(cookies_.begin(), cookies_.end(), [](const Cookie& cookie) { return cookie.value.empty(); }),
        cookies_.end());
}

std::string HttpCookies::GetCookieValue(_In_ const std::string& name) const {
    const std::string key = Trim(name);
    const auto it =
        std::find_if(cookies_.begin(), cookies_.end(), [&](const Cookie& cookie) { return cookie.name == key; });
    return it == cookies_.end() ? std::string{} : it->value;
}

std::vector<Cookie> HttpCookies::GetCookies(_In_ const std::string& name) const {
    const std::string key = Trim(name);
...
std::vector<std::string> HttpCookies::GetAllKeys(_In_ bool ignoreNull) const {
    std::vector<std::string> keys;
    for (const auto& cookie : cookies_) {
...
std::string HttpCookies::ToRequestCookieString(_In_ bool ignoreNull) const {
    std::ostringstream out;
    bool first = true;
    for (const auto& cookie : cookies_) {
        if (ignoreNull && cookie.value.empty()) {
...
std::vector<std::string> HttpCookies::ToSetCookieHeaders(_In_ bool ignoreNull) const {
    std::vector<std::string> result;
    for (const auto& cookie : cookies_) {
        if (ignoreNull && cookie.value.empty()) {
            continue;
...
std::vector<Cookie> HttpCookies::GetAllCookies(_In_ bool ignoreNull) const {
    std::vector<Cookie> result;
    for (const auto& cookie : cookies_) {
        if (ignoreNull && cookie.value.empty()) {
            continue;
...
RetryPolicy::RetryPolicy()
    : retryCurlCodes{CURLE_COULDNT_RESOLVE_PROXY,
...
MultipartPart MultipartPart::Field(_In_ std::string name, _In_ std::string value) {
    MultipartPart part;
...
MultipartPart MultipartPart::File(_In_ std::string name, _In_ std::string filePath, _In_ std::string contentType,
...
MemoryUploadSource::MemoryUploadSource(_In_ std::string data) : data_(std::move(data)) {}

std::size_t MemoryUploadSource::Read(_Out_ char* buffer, _In_ std::size_t capacity) {
    if (!buffer || capacity == 0 || offset_ >= data_.size())
        return 0;
...
std::optional<curl_off_t> MemoryUploadSource::GetSize() const {
    return static_cast<curl_off_t>(data_.size());
}

CallbackUploadSource::CallbackUploadSource(_In_ ReadCallback readCallback, _In_ RewindCallback rewindCallback,
                                           _In_ std::optional<curl_off_t> size)
    : read_(std::move(readCallback)), rewind_(std::move(rewindCallback)), size_(size) {}

std::size_t CallbackUploadSource::Read(_Out_ char* buffer, _In_ std::size_t capacity) {
...
std::optional<curl_off_t> CallbackUploadSource::GetSize() const {
    return size_;
}
...
FileUploadSource::FileUploadSource(_In_ std::string filePath)
    ...
    if (end >= 0)
        impl_->size = static_cast<curl_off_t>(end);
    impl_->stream.clear();
...
std::size_t FileUploadSource::Read(_Out_ char* buffer, _In_ std::size_t capacity) {
    if (!impl_ || !impl_->stream || !buffer || capacity == 0)
        return 0;
...
std::optional<curl_off_t> FileUploadSource::GetSize() const {
    return impl_ ? impl_->size : std::nullopt;
}

bool FileUploadSource::IsValid() const noexcept {
...
HttpErrorCategory ClassifyError(_In_ const HttpResponse& response) {
    if (response.errorCategory != HttpErrorCategory::None)
        return response.errorCategory;
    if (response.proxyConnectCode >= 400 && response.code == 0)
        return HttpErrorCategory::Proxy;
    if (response.curlCode == CURLE_OK) {
        return (response.code >= 400) ? HttpErrorCategory::Http : HttpErrorCategory::None;
    }
    switch (response.curlCode) {
    case CURLE_ABORTED_BY_CALLBACK:
        return HttpErrorCategory::Cancelled;
    case CURLE_COULDNT_RESOLVE_HOST:
        return HttpErrorCategory::Dns;
    case CURLE_COULDNT_CONNECT:
        return HttpErrorCategory::Connect;
#if LIBCURL_VERSION_NUM >= 0x074900
    case CURLE_PROXY:
#endif
    case CURLE_COULDNT_RESOLVE_PROXY:
    ...
    case CURLE_OPERATION_TIMEDOUT:
        return HttpErrorCategory::Timeout;
    case CURLE_READ_ERROR:
    case CURLE_SEND_ERROR:
    case CURLE_SEND_FAIL_REWIND:
        return HttpErrorCategory::Upload;
    ...
    case CURLE_TOO_MANY_REDIRECTS:
        return HttpErrorCategory::Http;
    case CURLE_BAD_FUNCTION_ARGUMENT:
    case CURLE_URL_MALFORMAT:
    case CURLE_UNSUPPORTED_PROTOCOL:
        return HttpErrorCategory::InvalidArgument;
    default:
        return HttpErrorCategory::Internal;
    }
}

std::string RedactHeaderValue(_In_ const DebugOptions& debug, _In_ std::string_view name, _In_ std::string_view value) {
    ...
    if (debug.hideSensitiveHeaders) {
        ...
        static const std::array<std::string_view, 9> builtins{"authorization", "proxy-authorization", "cookie",
    ...
    // 可能携带 userinfo、accessToken 或业务自定义 Query 凭据，即使调用方关闭了
    ...
    if (debug.hideSensitiveUrlData) {
        ...
        if (lower == "link")
            return "***";
        static const std::array<std::string_view, 5> urlBearingHeaders{"location", "content-location", "referer",
                                                                       "destination", "refresh"};
        for (auto item : urlBearingHeaders) {
            if (lower == item)
                return RedactUrlForDebug(debug, value);
        }
    }
...
std::string RedactUrlForDebug(_In_ const DebugOptions& debug, _In_ std::string_view input) {
    if (!debug.hideSensitiveUrlData)
    ...
    static const std::array<std::string_view, 10> builtins{"accessToken", "token",  "apiKey", "apikey",       "key",
                                                           "password",    "passwd", "secret", "clientSecret", "auth"};
    ...
    const auto isSensitive = [&](std::string_view encodedName) {
        ...
        for (const auto& item : debug.sensitiveQueryParameters) {
            if (decoded == ToLowerCopy(TrimCopy(item)))
                return true;
        }
        return false;
    };
    ...
    const bool hasQuery = query != std::string::npos && (fragment == std::string::npos || query < fragment);
    const std::size_t mainEnd = hasQuery ? query : (fragment == std::string::npos ? url.size() : fragment);
    ...
    if (fragment != std::string::npos) {
        output.push_back('#');
        const std::string_view fragmentText(url.data() + fragment + 1, url.size() - fragment - 1);
...
void EmitDebug(_In_ const DebugOptions& debug, _In_ const std::string& text) noexcept {
    if (!debug.enabled)
        return;
    try {
        if (debug.logger)
...
void LogRequestDebug(_In_ const HttpRequestData& request, _In_ const RequestOptions& options) noexcept {
    ...
    try {
        ...
        if (options.debug.logRequestHeaders) {
            for (const auto& h : options.headers.GetItems()) {
...
void LogResponseDebug(_In_ const HttpRequestData& request, _In_ const RequestOptions& options,
                      _In_ const HttpResponse& response) noexcept {
    if (!options.debug.enabled)
        return;
    try {
        std::ostringstream out;
        out << "===== HTTP RESPONSE " << request.requestId << " =====\n";
        out << "HTTP " << response.code << " CURL " << static_cast<int>(response.curlCode) << '\n';
        if (options.debug.logResponseHeaders) {
            for (const auto& h : response.headers.GetItems()) {
                out << h.name << ": " << RedactHeaderValue(options.debug, h.name, h.value) << '\n';
            }
        }
        if (options.debug.logResponseBody && !response.content.empty()) {
            ...
            if (n < response.content.size())
                out << "\n...[Body 已截断]";
            out << '\n';
        }
        out << "总耗时(us): " << response.totalTime.count() << " DNS(us): " << response.nameLookupTime.count()
            << " Connect(us): " << response.connectTime.count() << " TLS(us): " << response.tlsHandshakeTime.count()
            << " TTFB(us): " << response.firstByteTime.count() << " Attempts: " << response.attempts << '\n';
        EmitDebug(options.debug, out.str());
    } catch (...) {
...
HttpMetricsEvent MakeMetricsEvent(_In_ const HttpRequestData& request, _In_ const RequestOptions& options,
                                  _In_ const HttpResponse& response) {
    ...
    event.curlCode = response.curlCode;
    event.transportOk = response.IsTransportSuccessful();
...
void EmitMetricsSafely(_In_ const std::shared_ptr<IMetricsCollector>& collector, _In_ const HttpRequestData& request,
                       _In_ const RequestOptions& options, _In_ const HttpResponse& response) noexcept {
    if (!collector)
        return;
    try {
...
bool ValidateClientOptions(_In_ const HttpClientOptions& options, _Out_ HttpResponse& response) {
...
class RateLimiterState {
  public:
    explicit RateLimiterState(_In_ RateLimitPolicy policy)
        : policy_(std::move(policy)), tokens_(static_cast<double>((std::max)(policy_.burst, std::size_t{1}))),
          lastRefill_(_In_ std::chrono::steady_clock::now()) {}

    bool Acquire(_Inout_ std::atomic_bool* cancelFlag) {
        ...
        for (;;) {
            ...
            if (concurrencyOk && rateOk) {
                if (policy_.requestsPerSecond > 0.0)
                    tokens_ -= 1.0;
                ++active_;
                return true;
            }
...
class RateLease {
  public:
    explicit RateLease(_In_ std::shared_ptr<RateLimiterState> limiter) : limiter_(std::move(limiter)) {}
...
class CircuitBreakerState {
  public:
    explicit CircuitBreakerState(_In_ CircuitBreakerPolicy policy) : policy_(std::move(policy)) {}

    bool Allow(_Inout_ bool& halfOpenAdmission) {
        ...
        if (state_ == State::Open) {
            if (now - openedAt_ < policy_.openDuration)
                return false;
        ...
        if (state_ == State::HalfOpen) {
            if (halfOpenInFlight_ >= (std::max)(policy_.halfOpenMaxRequests, std::size_t{1}))
                return false;
            ++halfOpenInFlight_;
    ...
    void Cancel(_In_ bool halfOpenAdmission) {
        if (!halfOpenAdmission || policy_.failureThreshold == 0)
            return;
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void Record(_In_ const HttpResponse& response, _In_ bool halfOpenAdmission) {
        if (policy_.failureThreshold == 0)
            return;
        ...
        case HttpErrorCategory::Upload:
            ...
            relevant = response.curlCode == CURLE_SEND_ERROR;
            failed = relevant;
            break;
        case HttpErrorCategory::Download:
            // 写文件/用户回调失败不应打开服务熔断器。
            relevant = response.curlCode == CURLE_RECV_ERROR || response.curlCode == CURLE_PARTIAL_FILE ||
                       response.curlCode == CURLE_GOT_NOTHING;
            failed = relevant;
            break;
        ...
        if (state_ != State::Closed || !relevant)
            return;
        if (failed) {
            ++failures_;
            if (failures_ >= policy_.failureThreshold)
                OpenLocked();
        } else {
            failures_ = 0;
        }
...
bool CurlTransport::IsValid() const noexcept {
    return impl_ && impl_->curl;
}

CURL* CurlTransport::GetNativeHandle() noexcept {
    return impl_ ? impl_->curl : nullptr;
}

HttpResponse CurlTransport::Send(_In_ const HttpRequestData& request, _In_ const RequestOptions& options) {
    HttpResponse response;
    if (!impl_ || !impl_->curl) {
        response.curlCode = CURLE_FAILED_INIT;
...
std::vector<std::string> CurlTransport::GetCookieList() const {
    ...
    std::lock_guard<std::mutex> lock(impl_->mutex);
    curl_slist* list = nullptr;
    if (curl_easy_getinfo(impl_->curl, CURLINFO_COOKIELIST, &list) != CURLE_OK)
        return result;
...
void MockTransport::AddRule(_In_ MockPredicate predicate, _In_ MockResponder responder) {
    if (!predicate || !responder)
        return;
    std::lock_guard<std::mutex> lock(impl_->mutex);
    impl_->rules.push_back({std::move(predicate), std::move(responder)});
}

void MockTransport::AddStaticResponse(_In_ std::string method, _In_ std::string url, _In_ HttpResponse response) {
    AddRule(
        ...
        [response = std::move(response)](const HttpRequestData&, const RequestOptions&) mutable { return response; });
}
...
HttpResponse MockTransport::Send(_In_ const HttpRequestData& request, _In_ const RequestOptions& options) {
    try {
...
bool IAuthProvider::CanRefresh(_In_ const HttpResponse&) const {
    return false;
}

bool IAuthProvider::Refresh() {
    return false;
}

std::uint64_t IAuthProvider::Version() const noexcept {
    return 0;
}

BasicAuthProvider::BasicAuthProvider(_In_ std::string username, _In_ std::string password, _In_ unsigned long auth)
    : username_(std::move(username)), password_(std::move(password)), auth_(auth) {}

void BasicAuthProvider::Apply(_Inout_ HttpRequestData&, _Inout_ RequestOptions& options) {
...
BearerAuthProvider::BearerAuthProvider(_In_ std::string token, _In_ RefreshCallback refreshCallback)
    : token_(std::move(token)), refresh_(std::move(refreshCallback)) {}

void BearerAuthProvider::Apply(_Inout_ HttpRequestData&, _Inout_ RequestOptions& options) {
    options.headers.EraseHeader("Authorization");
    std::lock_guard<std::mutex> lock(mutex_);
...
bool BearerAuthProvider::CanRefresh(_In_ const HttpResponse& response) const {
...
void BearerAuthProvider::SetToken(_In_ std::string token) {
    std::lock_guard<std::mutex> lock(mutex_);
    token_ = std::move(token);
...
ApiKeyAuthProvider::ApiKeyAuthProvider(_In_ std::string name, _In_ std::string value, _In_ ApiKeyLocation location)
    ...
    if (location_ == ApiKeyLocation::Header && (!IsHttpToken(name_) || !IsSafeHttpFieldValue(value_))) {
        throw std::invalid_argument("API Key Header 名称或值不合法");
    }
}

void ApiKeyAuthProvider::Apply(_Out_ HttpRequestData& request, _Inout_ RequestOptions& options) {
    if (location_ == ApiKeyLocation::Header) {
        if (!options.headers.SetHeader(name_, value_)) {
            throw std::invalid_argument("API Key Header 名称或值不合法");
        }
        const std::string lowerName = ToLowerCopy(name_);
        const bool alreadySensitive =
            std::any_of(options.debug.sensitiveHeaders.begin(), options.debug.sensitiveHeaders.end(),
                        [&](const std::string& item) { return ToLowerCopy(TrimCopy(item)) == lowerName; });
        if (!alreadySensitive)
            options.debug.sensitiveHeaders.push_back(name_);
        return;
    }

    // Query 形式的 API Key 名称可能是业务自定义文本，不一定命中内置 token/apiKey 等规则。
    // 认证提供器既然知道这个参数承载凭据，就必须自动登记为敏感 Query，确保 Debug 与 Metrics 一致脱敏。
    const std::string lowerName = ToLowerCopy(name_);
    const bool alreadySensitive =
        std::any_of(options.debug.sensitiveQueryParameters.begin(), options.debug.sensitiveQueryParameters.end(),
                    [&](const std::string& item) { return ToLowerCopy(TrimCopy(item)) == lowerName; });
    if (!alreadySensitive)
        options.debug.sensitiveQueryParameters.push_back(name_);
    ...
    const std::size_t mainEnd = fragmentPos == std::string::npos ? request.url.size() : fragmentPos;
    const std::string fragment = fragmentPos == std::string::npos ? std::string{} : request.url.substr(fragmentPos);

    const std::size_t queryPos = request.url.find('?');
    const bool hasQuery = queryPos != std::string::npos && queryPos < mainEnd;
    const std::string prefix = hasQuery ? request.url.substr(0, queryPos) : request.url.substr(0, mainEnd);

    std::vector<std::string> kept;
    if (hasQuery) {
        const std::string_view queryText(request.url.data() + queryPos + 1, mainEnd - queryPos - 1);
        std::size_t start = 0;
        while (start <= queryText.size()) {
            const std::size_t amp = queryText.find('&', start);
            const std::size_t end = amp == std::string_view::npos ? queryText.size() : amp;
            const std::string_view item = queryText.substr(start, end - start);
            if (!item.empty()) {
                const std::size_t equal = item.find('=');
                const std::string decodedName =
                    UrlDecode(item.substr(0, equal == std::string_view::npos ? item.size() : equal));
...
bool IHttpMiddleware::Before(_Inout_ HttpRequestData&, _Inout_ RequestOptions&, _Inout_ HttpResponse&) {
    return true;
}

void IHttpMiddleware::After(_In_ const HttpRequestData&, _In_ const RequestOptions&, _Inout_ HttpResponse&) {}
...
void BasicMetricsCollector::OnRequestCompleted(_In_ const HttpMetricsEvent& event) {
    std::lock_guard<std::mutex> lock(impl_->mutex);
    auto& s = impl_->snapshot;
    ++s.totalRequests;
    if (event.transportOk)
        ++s.transportSuccess;
    else
        ++s.failedRequests;
    if (event.statusCode >= 200 && event.statusCode < 300)
        ...
        ++s.http4xx;
    else if (event.statusCode >= 500 && event.statusCode < 600)
        ++s.http5xx;
    if (event.attempts > 1)
        s.totalRetries += static_cast<std::uint64_t>(event.attempts - 1);
    if (event.downloadedBytes > 0)
        s.downloadedBytes += static_cast<std::uint64_t>(event.downloadedBytes);
    if (event.uploadedBytes > 0)
        s.uploadedBytes += static_cast<std::uint64_t>(event.uploadedBytes);
}

HttpMetricsSnapshot BasicMetricsCollector::GetSnapshot() const {
    std::lock_guard<std::mutex> lock(impl_->mutex);
...
HttpResponse HttpRequest::Send(_In_ HttpRequestData request, _In_ const RequestOptions& options) {
...
HttpResponse HttpRequest::Request(_In_ HttpMethod method, _In_ std::string_view url, _In_ std::string_view body,
                                  _In_ const RequestOptions& options) {
    return Request(ToString(method), url, body, options);
}

HttpResponse HttpRequest::Request(_In_ std::string_view method, _In_ std::string_view url, _In_ std::string_view body,
                                  _In_ const RequestOptions& options) {
...
HttpResponse HttpRequest::Get(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Get, url, {}, options);
}

HttpResponse HttpRequest::Head(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Head, url, {}, options);
}

HttpResponse HttpRequest::Post(_In_ std::string_view url, _In_ std::string_view body,
                               _In_ const RequestOptions& options) {
    return Request(HttpMethod::Post, url, body, options);
}

HttpResponse HttpRequest::Put(_In_ std::string_view url, _In_ std::string_view body,
                              _In_ const RequestOptions& options) {
    return Request(HttpMethod::Put, url, body, options);
}

HttpResponse HttpRequest::Delete(_In_ std::string_view url, _In_ std::string_view body,
                                 _In_ const RequestOptions& options) {
    return Request(HttpMethod::Delete, url, body, options);
}

HttpResponse HttpRequest::Patch(_In_ std::string_view url, _In_ std::string_view body,
                                _In_ const RequestOptions& options) {
    return Request(HttpMethod::Patch, url, body, options);
}

HttpResponse HttpRequest::Options(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Options, url, {}, options);
}

HttpResponse HttpRequest::Trace(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Trace, url, {}, options);
}

HttpResponse HttpRequest::Connect(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Connect, url, {}, options);
}

HttpResponse HttpRequest::RequestJson(_In_ std::string_view method, _In_ std::string_view url,
                                      _In_ std::string_view json, _In_ RequestOptions options) {
...
HttpResponse HttpRequest::PostJson(_In_ std::string_view url, _In_ std::string_view json, _In_ RequestOptions options) {
    return RequestJson("POST", url, json, std::move(options));
}

HttpResponse HttpRequest::PutJson(_In_ std::string_view url, _In_ std::string_view json, _In_ RequestOptions options) {
    return RequestJson("PUT", url, json, std::move(options));
}

HttpResponse HttpRequest::PatchJson(_In_ std::string_view url, _In_ std::string_view json,
                                    _In_ RequestOptions options) {
    return RequestJson("PATCH", url, json, std::move(options));
}

HttpResponse HttpRequest::PostForm(_In_ std::string_view url, _In_ const UrlParameters& form,
                                   _In_ RequestOptions options) {
    options.headers.SetDefaultHeader("Content-Type", "application/x-www-form-urlencoded");
    return Post(url, form.ToString(), options);
}

HttpResponse HttpRequest::UploadMultipart(_In_ std::string_view url, _In_ std::vector<MultipartPart> parts,
                                          _In_ RequestOptions options) {
    options.multipart = std::move(parts);
    return Post(url, {}, options);
}

HttpResponse HttpRequest::UploadFile(_In_ std::string_view url, _In_ const std::string& filePath,
                                     _In_ RequestOptions options, _In_ std::string_view method) {
...
HttpResponse HttpRequest::Download(_In_ std::string_view url, _In_ const std::string& filePath,
                                   _In_ RequestOptions options) {
    return DownloadWithInvoker(filePath, std::move(options),
                               [&](const RequestOptions& finalOptions) { return Get(url, finalOptions); });
}
...
void SetExtensionException(_Out_ HttpResponse& response, _In_ const std::string& message) {
    response.success = false;
    response.curlCode = CURLE_ABORTED_BY_CALLBACK;
    response.errorCategory = HttpErrorCategory::Internal;
    if (response.error.empty())
...
void RunMiddlewareAfterSafely(_In_ const std::vector<std::shared_ptr<IHttpMiddleware>>& middleware,
                              _In_ std::size_t passed, _In_ const HttpRequestData& request,
                              _In_ const RequestOptions& options, _Out_ HttpResponse& response) {
...
void RunResponseInterceptorsSafely(_In_ const std::vector<ResponseInterceptor>& interceptors,
                                   _In_ const HttpRequestData& request, _In_ const RequestOptions& options,
                                   _Out_ HttpResponse& response) {
...
struct HttpClient::Impl {
    ...
    Impl(_In_ RequestOptions defaults, _In_ HttpClientOptions client,
         _In_ std::shared_ptr<IHttpTransport> transportValue)
        : defaultOptions(std::move(defaults)), clientOptions(std::move(client)), transport(std::move(transportValue)),
    ...
    HttpResponse Execute(_In_ HttpRequestData request, _In_ RequestOptions options) {
        if (request.requestId.empty())
            request.requestId = GenerateRequestId();
        ...
        try {
            ...
            for (const auto& middleware : client.middleware) {
                ...
                if (!proceed) {
                    if (!response.success && response.curlCode == CURLE_OK && response.code == 0 &&
            ...
            if (client.authProvider && client.refreshAuthOnUnauthorized && client.authProvider->CanRefresh(response)) {
                std::lock_guard<std::mutex> refreshLock(authRefreshMutex);
                bool refreshed = client.authProvider->Version() != authVersion;
                if (!refreshed)
                    refreshed = client.authProvider->Refresh();
                if (refreshed && PrepareExternalSinkForLogicalReplay(options, response) &&
...
HttpClient::HttpClient() : impl_(_In_ std::make_unique<Impl>(RequestOptions{}, HttpClientOptions{}, nullptr)) {}

HttpClient::HttpClient(_In_ RequestOptions defaultOptions)
    : impl_(_In_ std::make_unique<Impl>(std::move(defaultOptions), HttpClientOptions{}, nullptr)) {}

HttpClient::HttpClient(_In_ HttpClientOptions clientOptions, _In_ std::shared_ptr<IHttpTransport> transport)
    : impl_(_In_ std::make_unique<Impl>(RequestOptions{}, std::move(clientOptions), std::move(transport))) {}

HttpClient::HttpClient(_In_ RequestOptions defaultOptions, _In_ HttpClientOptions clientOptions,
                       _In_ std::shared_ptr<IHttpTransport> transport)
    : impl_(std::make_unique<Impl>(std::move(defaultOptions), std::move(clientOptions), std::move(transport))) {}
...
RequestOptions& HttpClient::GetDefaultOptions() noexcept {
    return impl_->defaultOptions;
}

const RequestOptions& HttpClient::GetDefaultOptions() const noexcept {
    return impl_->defaultOptions;
}

void HttpClient::SetDefaultOptions(_In_ RequestOptions options) {
    std::lock_guard<std::mutex> lock(impl_->configMutex);
    impl_->defaultOptions = std::move(options);
}

const HttpClientOptions& HttpClient::GetClientOptions() const noexcept {
    return impl_->clientOptions;
}

void HttpClient::SetClientOptions(_In_ HttpClientOptions options) {
    std::lock_guard<std::mutex> lock(impl_->configMutex);
...
void HttpClient::SetTransport(_In_ std::shared_ptr<IHttpTransport> transport) {
    if (!transport)
        transport = std::make_shared<CurlTransport>();
    std::lock_guard<std::mutex> lock(impl_->configMutex);
...
void HttpClient::AddMiddleware(_In_ std::shared_ptr<IHttpMiddleware> middleware) {
    if (!middleware)
        return;
    std::lock_guard<std::mutex> lock(impl_->configMutex);
    impl_->clientOptions.middleware.push_back(std::move(middleware));
}

void HttpClient::AddRequestInterceptor(_In_ RequestInterceptor interceptor) {
    if (!interceptor)
        return;
    std::lock_guard<std::mutex> lock(impl_->configMutex);
    impl_->clientOptions.requestInterceptors.push_back(std::move(interceptor));
}

void HttpClient::AddResponseInterceptor(_In_ ResponseInterceptor interceptor) {
    if (!interceptor)
        return;
    std::lock_guard<std::mutex> lock(impl_->configMutex);
    impl_->clientOptions.responseInterceptors.push_back(std::move(interceptor));
}

HttpResponse HttpClient::Send(_In_ HttpRequestData request) {
    RequestOptions options;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        options = impl_->defaultOptions;
    }
    return impl_->Execute(std::move(request), std::move(options));
}

HttpResponse HttpClient::Send(_In_ HttpRequestData request, _In_ const RequestOptions& options) {
    return impl_->Execute(std::move(request), options);
}

HttpResponse HttpClient::Request(_In_ HttpMethod method, _In_ std::string_view url, _In_ std::string_view body) {
    return Request(ToString(method), url, body);
}
...
HttpResponse HttpClient::Request(_In_ std::string_view method, _In_ std::string_view url, _In_ std::string_view body) {
    HttpRequestData r;
    r.method.assign(method.data(), method.size());
    r.url.assign(url.data(), url.size());
    r.body.assign(body.data(), body.size());
...
HttpResponse HttpClient::Request(_In_ std::string_view method, _In_ std::string_view url, _In_ std::string_view body,
    ...
    r.url.assign(url.data(), url.size());
    r.body.assign(body.data(), body.size());
    return Send(std::move(r), options);
}

HttpResponse HttpClient::Get(_In_ std::string_view url) {
    return Request(HttpMethod::Get, url);
}

HttpResponse HttpClient::Get(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Get, url, {}, options);
}

HttpResponse HttpClient::Head(_In_ std::string_view url) {
    return Request(HttpMethod::Head, url);
}

HttpResponse HttpClient::Head(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Head, url, {}, options);
}

HttpResponse HttpClient::Post(_In_ std::string_view url, _In_ std::string_view body) {
...
HttpResponse HttpClient::Put(_In_ std::string_view url, _In_ std::string_view body) {
...
HttpResponse HttpClient::Delete(_In_ std::string_view url, _In_ std::string_view body) {
    return Request(HttpMethod::Delete, url, body);
}
...
HttpResponse HttpClient::Patch(_In_ std::string_view url, _In_ std::string_view body) {
...
HttpResponse HttpClient::Options(_In_ std::string_view url) {
    return Request(HttpMethod::Options, url);
}

HttpResponse HttpClient::Options(_In_ std::string_view url, _In_ const RequestOptions& options) {
    return Request(HttpMethod::Options, url, {}, options);
}

HttpResponse HttpClient::RequestJson(_In_ std::string_view method, _In_ std::string_view url,
                                     _In_ std::string_view json) {
    RequestOptions o;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        o = impl_->defaultOptions;
    }
    return RequestJson(method, url, json, std::move(o));
}
...
HttpResponse HttpClient::PostJson(_In_ std::string_view url, _In_ std::string_view json) {
    return RequestJson("POST", url, json);
}

HttpResponse HttpClient::PostJson(_In_ std::string_view url, _In_ std::string_view json, _In_ RequestOptions options) {
    return RequestJson("POST", url, json, std::move(options));
}

HttpResponse HttpClient::PutJson(_In_ std::string_view url, _In_ std::string_view json) {
    return RequestJson("PUT", url, json);
}

HttpResponse HttpClient::PutJson(_In_ std::string_view url, _In_ std::string_view json, _In_ RequestOptions options) {
    return RequestJson("PUT", url, json, std::move(options));
}

HttpResponse HttpClient::PatchJson(_In_ std::string_view url, _In_ std::string_view json) {
    return RequestJson("PATCH", url, json);
}

HttpResponse HttpClient::PatchJson(_In_ std::string_view url, _In_ std::string_view json, _In_ RequestOptions options) {
    return RequestJson("PATCH", url, json, std::move(options));
}

HttpResponse HttpClient::PostForm(_In_ std::string_view url, _In_ const UrlParameters& form) {
    RequestOptions o;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        o = impl_->defaultOptions;
    }
    return PostForm(url, form, std::move(o));
}
...
HttpResponse HttpClient::UploadMultipart(_In_ std::string_view url, _In_ std::vector<MultipartPart> parts) {
    RequestOptions o;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        o = impl_->defaultOptions;
    }
    return UploadMultipart(url, std::move(parts), std::move(o));
}
...
HttpResponse HttpClient::UploadFile(_In_ std::string_view url, _In_ const std::string& filePath,
                                    _In_ std::string_view method) {
    RequestOptions o;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        o = impl_->defaultOptions;
    }
    return UploadFile(url, filePath, std::move(o), method);
}
...
HttpResponse HttpClient::UploadFile(_In_ std::string_view url, _In_ const std::string& filePath,
    ...
    if (!source->IsValid()) {
        HttpResponse r;
        r.curlCode = CURLE_READ_ERROR;
        r.error = "无法打开上传文件: " + filePath;
        r.errorCategory = HttpErrorCategory::Upload;
        return r;
    }
    options.uploadSource = source;
    return Request(method, url, {}, options);
...
HttpResponse HttpClient::Download(_In_ std::string_view url, _In_ const std::string& filePath) {
    RequestOptions o;
    {
        std::lock_guard<std::mutex> lock(impl_->configMutex);
        o = impl_->defaultOptions;
    }
    return Download(url, filePath, std::move(o));
}
...
std::shared_ptr<IHttpTransport> HttpClient::GetTransport() const {
    std::lock_guard<std::mutex> lock(impl_->configMutex);
    return impl_->transport;
}

std::vector<std::string> HttpClient::GetCookieList() const {
    auto t = std::dynamic_pointer_cast<CurlTransport>(GetTransport());
    return t ? t->GetCookieList() : std::vector<std::string>{};
}

bool HttpClient::ClearCookies() {
    auto t = std::dynamic_pointer_cast<CurlTransport>(GetTransport());
    return t && t->ClearCookies();
}

bool HttpClient::ClearSessionCookies() {
    auto t = std::dynamic_pointer_cast<CurlTransport>(GetTransport());
    return t && t->ClearSessionCookies();
}

bool HttpClient::FlushCookies() {
    auto t = std::dynamic_pointer_cast<CurlTransport>(GetTransport());
    return t && t->FlushCookies();
}
...
class AsyncCompletionDispatcher {
  public:
    AsyncCompletionDispatcher() {
        ...
        const std::size_t workerCount =
            (std::max)(std::size_t{2},
                       (std::min)(std::size_t{4}, hardware == 0 ? std::size_t{2} : static_cast<std::size_t>(hardware)));
    ...
    void Post(_In_ std::function<void()> task) {
        if (!task)
            return;
        {
            std::lock_guard<std::mutex> lock(mutex_);
...
AsyncCompletionDispatcher& GetCompletionDispatcher() {
    static AsyncCompletionDispatcher dispatcher;
    return dispatcher;
}

template <typename Poster>
void DispatchAsyncCompletionWith(_In_ AsyncCompletionCallback callback, _In_ HttpResponse response,
                                 _Inout_ Poster&& poster) noexcept {
    if (!callback)
        return;
...
void DispatchAsyncCompletion(_In_ AsyncCompletionCallback callback, _In_ HttpResponse response) noexcept {
    DispatchAsyncCompletionWith(std::move(callback), std::move(response),
                                [](const std::function<void()>& task) { GetCompletionDispatcher().Post(task); });
}
...
struct AsyncHttpClient::Impl {
    struct Operation {
        ...
        Operation(_In_ HttpRequestData r, _In_ RequestOptions o, _In_ AsyncCompletionCallback cb = {})
        ...
        void ResetHolders() {
            if (headerList.value) {
                curl_slist_free_all(headerList.value);
                headerList.value = nullptr;
            }
            if (resolveList.value) {
                curl_slist_free_all(resolveList.value);
                resolveList.value = nullptr;
            }
            if (mime.value) {
                curl_mime_free(mime.value);
                mime.value = nullptr;
            }
        }
    };
    ...
    Impl(_In_ RequestOptions defaults, _In_ HttpClientOptions client)
        ...
        if (!EnsureGlobal()) {
            ...
            if (!multi)
                initializationError = "curl_multi_init 失败";
            if (!cookieShare && initializationError.empty())
                initializationError = "curl_share_init 失败";
            if (cookieShare) {
        ...
        if (multi && cookieShare && initializationError.empty()) {
            ...
            multiOptionsOk =
                setMultiOption(CURLMOPT_PIPELINING, static_cast<long>(CURLPIPE_MULTIPLEX), "CURLMOPT_PIPELINING");
#endif
#if LIBCURL_VERSION_NUM >= 0x071E00
            if (multiOptionsOk && clientOptions.maxTotalConnections > 0)
                multiOptionsOk = setMultiOption(CURLMOPT_MAX_TOTAL_CONNECTIONS, clientOptions.maxTotalConnections,
                                                "CURLMOPT_MAX_TOTAL_CONNECTIONS");
            if (multiOptionsOk && clientOptions.maxHostConnections > 0)
                multiOptionsOk = setMultiOption(CURLMOPT_MAX_HOST_CONNECTIONS, clientOptions.maxHostConnections,
                                                "CURLMOPT_MAX_HOST_CONNECTIONS");
#endif
#if LIBCURL_VERSION_NUM >= 0x074300
            if (multiOptionsOk && clientOptions.maxConcurrentStreams > 0)
                multiOptionsOk = setMultiOption(CURLMOPT_MAX_CONCURRENT_STREAMS, clientOptions.maxConcurrentStreams,
                                                "CURLMOPT_MAX_CONCURRENT_STREAMS");
#endif
            if (multiOptionsOk) {
                try {
                    // 在 AsyncHttpClient 完成构造前初始化完成分发器，避免全局/静态客户端析构时出现静态销毁顺序问题。
                    (void)GetCompletionDispatcher();
    ...
    ~Impl() {
        ...
        for (auto& resources : quarantinedCurlResources) {
            if (resources.headerList)
                curl_slist_free_all(resources.headerList);
            if (resources.resolveList)
                curl_slist_free_all(resources.resolveList);
            if (resources.mime)
                curl_mime_free(resources.mime);
            if (resources.easy)
                curl_easy_cleanup(resources.easy);
        }
    ...
    void QuarantineCurlResources(_Inout_ Operation& op) {
    ...
    bool DetachOrQuarantine(_Inout_ Operation& op, _Inout_ CURL* easy) noexcept {
        if (!easy || !multi)
            return true;
    ...
    HttpResponse MakeMultiFailure(_In_ std::string requestId = {}) const {
        HttpResponse response;
        response.success = false;
        response.curlCode = CURLE_FAILED_INIT;
        response.errorCategory = HttpErrorCategory::Internal;
    ...
    void MarkMultiFatal(_In_ CURLMcode code, _In_ const char* operation) {
        if (code == CURLM_OK)
            return;
        const int expectedOk = static_cast<int>(CURLM_OK);
        int expected = expectedOk;
        if (fatalMultiCode.compare_exchange_strong(expected, static_cast<int>(code), std::memory_order_acq_rel,
                                                   std::memory_order_acquire)) {
            std::lock_guard<std::mutex> lock(fatalMultiMutex);
            fatalMultiMessage =
                std::string(operation) + " 失败，curl_multi 状态不可继续使用: " + curl_multi_strerror(code);
        }
    }
    ...
    std::future<HttpResponse> MakeReadyFuture(_In_ HttpResponse response) {
    ...
    bool Preprocess(_Out_ HttpRequestData& request, _Inout_ RequestOptions& options, _Inout_ HttpResponse& early,
                    _Inout_ std::uint64_t& authVersion) {
        if (request.requestId.empty())
            request.requestId = GenerateRequestId();
        ...
        std::size_t passed = 0;
        try {
            for (auto& interceptor : clientOptions.requestInterceptors)
                if (interceptor)
                    interceptor(request, options);
            ...
            for (const auto& m : clientOptions.middleware) {
                ...
                if (!m->Before(request, options, early)) {
                    if (!early.success && early.curlCode == CURLE_OK && early.code == 0 && early.error.empty()) {
                        early.curlCode = CURLE_ABORTED_BY_CALLBACK;
    ...
    std::future<HttpResponse> Submit(_In_ HttpRequestData request, _In_ RequestOptions options) {
        if (request.requestId.empty())
            request.requestId = GenerateRequestId();
        if (shuttingDown.load(std::memory_order_acquire)) {
            HttpResponse r;
            r.requestId = request.requestId;
            r.curlCode = CURLE_ABORTED_BY_CALLBACK;
            r.error = "AsyncHttpClient 正在关闭";
            r.errorCategory = HttpErrorCategory::Cancelled;
            return MakeReadyFuture(std::move(r));
        }
        if (!multi || !cookieShare) {
            HttpResponse r;
            r.requestId = request.requestId;
            r.curlCode = CURLE_FAILED_INIT;
            r.error = initializationError.empty() ? "curl_multi 或 Cookie Share 初始化失败" : initializationError;
            r.errorCategory = HttpErrorCategory::Internal;
            return MakeReadyFuture(std::move(r));
        }
        if (HasFatalMultiError())
            return MakeReadyFuture(MakeMultiFailure(request.requestId));
        HttpResponse early;
        std::uint64_t authVersion = 0;
        if (!Preprocess(request, options, early, authVersion))
            return MakeReadyFuture(std::move(early));
        ...
        pendingCount->fetch_add(1, std::memory_order_relaxed);
        {
            ...
            if (shuttingDown.load(std::memory_order_acquire) || HasFatalMultiError()) {
                ...
                HttpResponse rejected = shuttingDown.load(_In_ std::memory_order_acquire)
                                            ? HttpResponse{}
                                            : MakeMultiFailure(op->request.requestId);
                if (shuttingDown.load(std::memory_order_acquire)) {
                    rejected.curlCode = CURLE_ABORTED_BY_CALLBACK;
                    rejected.error = "AsyncHttpClient 正在关闭";
                    rejected.errorCategory = HttpErrorCategory::Cancelled;
                }
                try {
                    op->promise.set_value(std::move(rejected));
                } catch (...) {
                }
                return f;
            }
    ...
    void SubmitCallback(_In_ HttpRequestData request, _In_ RequestOptions options,
        ...
        if (shuttingDown.load(std::memory_order_acquire)) {
            ...
            r.error = "AsyncHttpClient 正在关闭";
            r.errorCategory = HttpErrorCategory::Cancelled;
            DispatchAsyncCompletion(std::move(callback), std::move(r));
            return;
        }
        if (!multi || !cookieShare) {
            HttpResponse r;
        ...
        if (HasFatalMultiError()) {
            DispatchAsyncCompletion(std::move(callback), MakeMultiFailure(request.requestId));
            return;
        }
        HttpResponse early;
        std::uint64_t authVersion = 0;
        if (!Preprocess(request, options, early, authVersion)) {
            DispatchAsyncCompletion(std::move(callback), std::move(early));
            return;
        }
    ...
    void ReleaseAdmissionForAuthPause(_Inout_ Operation& op) {
    ...
    void QueueAuthRefresh(_In_ std::unique_ptr<Operation> op) {
        if (!op)
            return;
        ...
        if (op->easy && op->options.enableCookieEngine) {
            ...
            if (detachCode != CURLE_OK) {
                ...
                response.success = false;
                response.curlCode = detachCode;
                response.errorCategory = HttpErrorCategory::Internal;
                response.error = std::string("认证刷新前解除 Cookie Share 绑定失败: ") + curl_easy_strerror(detachCode);
                Complete(std::move(op), std::move(response));
                return;
            }
        }
    ...
    void HandleAuthRefresh(_In_ std::unique_ptr<Operation> op) {
        if (!op)
            return;
        if (HasFatalMultiError()) {
            HttpResponse r = MakeMultiFailure(op->request.requestId);
        ...
        try {
            bool refreshed = clientOptions.authProvider && clientOptions.authProvider->Version() != op->authVersion;
            ...
            op->response.curlCode = CURLE_ABORTED_BY_CALLBACK;
    ...
    void AuthLoop() noexcept {
        for (;;) {
            std::unique_ptr<Operation> op;
            {
                ...
                if (authStopping && authQueue.empty())
                    return;
    ...
    bool Prepare(_Inout_ Operation& op) {
        op.transferStarted = false;
        if (!op.easy) {
            op.response.curlCode = CURLE_FAILED_INIT;
            op.response.error = "curl_easy_init 失败";
            return false;
        }
        op.ResetHolders();
        curl_easy_reset(op.easy);
        if (op.options.enableCookieEngine && curl_easy_setopt(op.easy, CURLOPT_SHARE, cookieShare) != CURLE_OK) {
            op.response.curlCode = CURLE_FAILED_INIT;
        ...
        if (op.totalAttempts > 0 && op.options.uploadSource) {
            try {
                if (!op.options.uploadSource->Rewind()) {
                    op.response.curlCode = CURLE_SEND_FAIL_REWIND;
        ...
        if (!ConfigureRequest(op.easy, op.request.method, op.request.url, op.request.body, op.options, op.context,
                              op.headerList, op.resolveList, op.mime, op.errorBuffer, op.response))
            return false;
        ++op.totalAttempts;
        ...
        if (freshCode != CURLE_OK) {
            op.response.curlCode = freshCode;
            op.response.errorCategory = HttpErrorCategory::Internal;
    ...
    void Complete(_In_ std::unique_ptr<Operation> op, _In_ HttpResponse response) {
        if (!op)
            return;
        if (op->rateHeld && rateLimiter) {
            rateLimiter->Release();
        ...
        if (!op->completion && clientOptions.middleware.empty() && clientOptions.responseInterceptors.empty() &&
            !clientOptions.metricsCollector && !op->options.debug.enabled) {
            // 先移出 Pending 集合，再兑现 future；这样 future.get() 返回后 GetPendingCount 不会仍短暂包含该请求。
        ...
        std::function<void()> task =
            [middleware = std::move(middleware), interceptors = std::move(interceptors), metrics = std::move(metrics),
             request = std::move(request), options = std::move(options), response = std::move(response),
             completion = std::move(completion), promise = std::move(promise), counter = std::move(counter)]() mutable {
                ...
                if (completion) {
                    ...
                    // 与 callback 路径保持同一“交付边界”：先从 GetPendingCount 移除，再令 future 就绪。
        ...
        try {
            GetCompletionDispatcher().Post(task);
        } catch (...) {
    ...
    void QueueDelay(_In_ std::unique_ptr<Operation> op, _In_ std::chrono::milliseconds delay) {
        const auto now = std::chrono::steady_clock::now();
        const auto maxDelay =
            std::chrono::duration_cast<std::chrono::milliseconds>((std::chrono::steady_clock::time_point::max)() - now);
        op->readyAt = delay >= maxDelay ? (std::chrono::steady_clock::time_point::max)() : now + delay;
        delayed.push_back(std::move(op));
    }

    void FinalizeTransfer(_In_ std::unique_ptr<Operation> op, _In_ CURLcode result) {
        if (!op)
            return;
        op->response.curlCode = result;
        ...
        op->response.attempts = op->totalAttempts;
        if (op->response.curlCode != CURLE_OK) {
            ...
            } else if (op->errorBuffer[0] != '\0')
                op->response.error = op->errorBuffer.data();
            else if (op->response.error.empty())
                op->response.error = curl_easy_strerror(op->response.curlCode);
        }

        try {
            if (!op->authReplayUsed && clientOptions.authProvider && clientOptions.refreshAuthOnUnauthorized &&
    ...
    void StartReadyOperations() {
        ...
        while (!local.empty()) {
            delayed.push_back(std::move(local.front()));
            local.pop_front();
        }
        const auto now = std::chrono::steady_clock::now();
        for (std::size_t i = 0; i < delayed.size();) {
            ...
            if (op->options.cancelFlag && op->options.cancelFlag->load(std::memory_order_relaxed)) {
                ...
                r.error = "请求已取消";
                r.errorCategory = HttpErrorCategory::Cancelled;
                auto done = std::move(op);
                delayed.erase(delayed.begin() + static_cast<std::ptrdiff_t>(i));
                Complete(std::move(done), std::move(r));
                continue;
            }
            if (!op->rateHeld) {
                if (rateLimiter && !rateLimiter->TryAcquire()) {
                    op->readyAt = now + std::chrono::milliseconds(10);
                    ++i;
                    continue;
                }
            ...
            if (!op->circuitAdmitted) {
                if (circuitBreaker && !circuitBreaker->Allow(op->circuitHalfOpenAdmission)) {
                    HttpResponse r;
                    r.curlCode = CURLE_OK;
                    r.error = "Circuit Breaker 当前处于熔断状态";
                    r.errorCategory = HttpErrorCategory::CircuitOpen;
                    auto done = std::move(op);
                    delayed.erase(delayed.begin() + static_cast<std::ptrdiff_t>(i));
                    Complete(std::move(done), std::move(r));
                    continue;
            ...
            if (!Prepare(*op)) {
                auto done = std::move(op);
                HttpResponse response = std::move(done->response);
                delayed.erase(delayed.begin() + static_cast<std::ptrdiff_t>(i));
                Complete(std::move(done), std::move(response));
                continue;
            }
            ...
            if (mc != CURLM_OK) {
                HttpResponse r = std::move(op->response);
                r.curlCode = CURLE_FAILED_INIT;
                r.error = curl_multi_strerror(mc);
                auto done = std::move(op);
                delayed.erase(delayed.begin() + static_cast<std::ptrdiff_t>(i));
                Complete(std::move(done), std::move(r));
                continue;
    ...
    void WorkerLoop() {
        ...
        while (!stopping.load(std::memory_order_acquire)) {
            ...
            if (active.empty()) {
                ...
                cv.wait_for(lock, std::chrono::milliseconds(incoming.empty() && delayed.empty() ? 50 : 10));
            } else {
                int numfds = 0;
#if LIBCURL_VERSION_NUM >= 0x074200
...
AsyncHttpClient::AsyncHttpClient() : impl_(_In_ std::make_unique<Impl>(RequestOptions{}, HttpClientOptions{})) {}

AsyncHttpClient::AsyncHttpClient(_In_ RequestOptions defaultOptions)
    : impl_(_In_ std::make_unique<Impl>(std::move(defaultOptions), HttpClientOptions{})) {}

AsyncHttpClient::AsyncHttpClient(_In_ RequestOptions defaultOptions, _In_ HttpClientOptions clientOptions)
    : impl_(std::make_unique<Impl>(std::move(defaultOptions), std::move(clientOptions))) {}

AsyncHttpClient::~AsyncHttpClient() = default;

std::future<HttpResponse> AsyncHttpClient::SendAsync(_In_ HttpRequestData request, _In_ RequestOptions options) {
    return impl_->Submit(std::move(request), std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::SendAsync(_In_ HttpRequestData request) {
    return impl_->Submit(std::move(request), impl_->defaultOptions);
}

void AsyncHttpClient::SendWithCallback(_In_ HttpRequestData request, _In_ RequestOptions options,
                                       _In_ AsyncCompletionCallback callback) {
    impl_->SubmitCallback(std::move(request), std::move(options), std::move(callback));
}

std::future<HttpResponse> AsyncHttpClient::GetAsync(_In_ std::string_view url, _In_ RequestOptions options) {
    return RequestAsync("GET", url, {}, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::GetAsync(_In_ std::string_view url) {
    return RequestAsync("GET", url, {}, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::HeadAsync(_In_ std::string_view url, _In_ RequestOptions options) {
    return RequestAsync("HEAD", url, {}, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::HeadAsync(_In_ std::string_view url) {
    return RequestAsync("HEAD", url, {}, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::PostAsync(_In_ std::string_view url, _In_ std::string_view body,
                                                     _In_ RequestOptions options) {
    return RequestAsync("POST", url, body, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::PostAsync(_In_ std::string_view url, _In_ std::string_view body) {
    return RequestAsync("POST", url, body, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::PutAsync(_In_ std::string_view url, _In_ std::string_view body,
                                                    _In_ RequestOptions options) {
    return RequestAsync("PUT", url, body, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::PutAsync(_In_ std::string_view url, _In_ std::string_view body) {
    return RequestAsync("PUT", url, body, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::DeleteAsync(_In_ std::string_view url, _In_ std::string_view body,
                                                       _In_ RequestOptions options) {
    return RequestAsync("DELETE", url, body, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::DeleteAsync(_In_ std::string_view url, _In_ std::string_view body) {
    return RequestAsync("DELETE", url, body, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::PatchAsync(_In_ std::string_view url, _In_ std::string_view body,
                                                      _In_ RequestOptions options) {
    return RequestAsync("PATCH", url, body, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::PatchAsync(_In_ std::string_view url, _In_ std::string_view body) {
    return RequestAsync("PATCH", url, body, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::OptionsAsync(_In_ std::string_view url, _In_ RequestOptions options) {
    return RequestAsync("OPTIONS", url, {}, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::OptionsAsync(_In_ std::string_view url) {
    return RequestAsync("OPTIONS", url, {}, impl_->defaultOptions);
}

std::future<HttpResponse> AsyncHttpClient::PostJsonAsync(_In_ std::string_view url, _In_ std::string_view json,
                                                         _In_ RequestOptions options) {
    options.headers.SetDefaultHeader("Content-Type", "application/json");
    options.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("POST", url, json, std::move(options));
}

std::future<HttpResponse> AsyncHttpClient::PostJsonAsync(_In_ std::string_view url, _In_ std::string_view json) {
    auto o = impl_->defaultOptions;
    o.headers.SetDefaultHeader("Content-Type", "application/json");
    o.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("POST", url, json, std::move(o));
}

std::future<HttpResponse> AsyncHttpClient::PutJsonAsync(_In_ std::string_view url, _In_ std::string_view json,
                                                        _In_ RequestOptions options) {
    options.headers.SetDefaultHeader("Content-Type", "application/json");
    options.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("PUT", url, json, std::move(options));
}
...
std::future<HttpResponse> AsyncHttpClient::PutJsonAsync(_In_ std::string_view url, _In_ std::string_view json) {
    ...
    o.headers.SetDefaultHeader("Content-Type", "application/json");
    o.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("PUT", url, json, std::move(o));
}

std::future<HttpResponse> AsyncHttpClient::PatchJsonAsync(_In_ std::string_view url, _In_ std::string_view json,
                                                          _In_ RequestOptions options) {
    options.headers.SetDefaultHeader("Content-Type", "application/json");
    options.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("PATCH", url, json, std::move(options));
}
...
std::future<HttpResponse> AsyncHttpClient::PatchJsonAsync(_In_ std::string_view url, _In_ std::string_view json) {
    ...
    o.headers.SetDefaultHeader("Content-Type", "application/json");
    o.headers.SetDefaultHeader("Accept", "application/json");
    return RequestAsync("PATCH", url, json, std::move(o));
}

std::future<HttpResponse> AsyncHttpClient::RequestAsync(_In_ std::string_view method, _In_ std::string_view url,
...
std::future<HttpResponse> AsyncHttpClient::RequestAsync(_In_ std::string_view method, _In_ std::string_view url,
    ...
    return RequestAsync(method, url, body, impl_->defaultOptions);
}

std::size_t AsyncHttpClient::GetPendingCount() const {
    return impl_->pendingCount->load(std::memory_order_relaxed);
}
...
struct HttpAwaitable::State {
    ...
    void Complete(_In_ HttpResponse value) {
...
HttpAwaitable::HttpAwaitable(_In_ std::shared_ptr<State> state) : state_(std::move(state)) {}
...
HttpAwaitable::HttpAwaitable(HttpAwaitable&& other) noexcept : state_(_In_ std::move(other.state_)) {}
...
bool HttpAwaitable::await_suspend(_In_ std::coroutine_handle<> handle) {
    std::lock_guard<std::recursive_mutex> lock(state_->mutex);
    if (state_->response)
        return false;
...
HttpAwaitable GetAwaitable(_Inout_ AsyncHttpClient& client, _In_ std::string_view url, _In_ RequestOptions options) {
    auto state = std::make_shared<HttpAwaitable::State>();
    HttpRequestData r;
    r.method = "GET";
    r.url.assign(url.data(), url.size());
    client.SendWithCallback(std::move(r), std::move(options),
                            [state](HttpResponse response) { state->Complete(std::move(response)); });
