...
### IsTransportSuccessful()

```cpp
if (!response.IsTransportSuccessful()) {
...
```cpp
UrlParameters params("a=1&a=2&empty=&flag");
```
...
普通 Header 推荐优先使用 `HttpHeaders`，因为它会进行名称和值的安全检查。
...
注意：`HttpCookies` 是结构化便利容器；真正的 Session Cookie 路由和 Cookie Jar 更推荐让 libcurl Cookie Engine 管理。
...
为了查询方便，Trailer 也会合并进 `response.headers`，但不会覆盖 `finalRawHeaders` 的主响应 Header 语义；Trailer 中的 `Set-Cookie` 不会被误当成普通响应 Cookie。
...
| 字段 | 作用 / 语义 |
|---|---|
| `success` | libcurl/传输层是否成功完成。它不代表 HTTP 一定是 2xx；推荐通过 `IsTransportSuccessful()` 判断传输成功。 |
| `error` | 封装层或 libcurl 的错误描述。成功时通常为空。 |
| `content` | 内存中的响应 Body；是否保存受 `storeResponseBody`、流式 sink 和大小上限影响。 |
| `code` | 最终 HTTP 状态码，例如 200、404。 |
| `originalHeaders` | 本次请求收到的全部原始 Header 块，包括重定向链中的中间响应。 |
...
| `curlCode` | 原始 libcurl `CURLcode`；网络/TLS/超时等底层错误排查首先看它。 |
| `errorCategory` | 封装层粗粒度错误分类 `HttpErrorCategory`。 |
| `requestId` | 请求追踪 ID；请求未提供时封装会生成。 |
| `finalRawHeaders` | 最终常规响应 Header 块的原始文本。 |
...
```text
IsTransportSuccessful() = success && curlCode == CURLE_OK
IsSuccessful()          = IsTransportSuccessful() && code >= 200 && code < 300
```
...
会使用你传入的 `options`，不会自动把它与 `GetDefaultOptions()` 深度合并。
...
```cpp
auto count = client.GetPendingCount();

// 近实时统计排队、延迟重试、传输、认证刷新和完成分发等待中的请求。
// future 置为 ready 之前、completion callback 开始之前，会先把该请求移出计数。
```

`GetPendingCount()` 是观测指标，不是等待全部请求完成的同步原语。计数递减与 `promise.set_value()` / callback 调用之间存在极短的执行窗口，因此业务需要确认某个请求完成时必须等待对应 future、callback 或 coroutine 结果，而不是轮询 `GetPendingCount()==0`。
...
`SetDefaultOptions()`、`SetClientOptions()`、`SetTransport()`、`Add*()` 等受控写接口内部有互斥保护，请求执行时也会复制配置快照。**但 `GetDefaultOptions()` / `GetClientOptions()` 返回的是内部对象引用，不是加锁快照**：直接通过可写 `GetDefaultOptions()` 修改，或者在其他线程调用 Setter/Add* 的同时读取这些引用，仍可能形成数据竞争。直接引用接口只应在没有并发配置修改/请求的初始化阶段使用；运行期动态配置优先使用 Setter，并由业务层保证配置读取者与写入者的同步边界。
...
源码文件名延续项目既有命名，不在文件名里加版本号；当前接口以 `curl_ex.h`、`curl_ex.cpp` 和本 `README.md` 为准。
...
## 70. 在已安装的 ytpp_cpp_lib 中使用

本项目的 `vcpkg.json` 负责构建本库时解析 libcurl。安装本项目后，消费方使用导出的 CMake 包：
...
头文件使用 `#include <curl_ex/curl_ex.h>`。依赖解析以消费方选择的 vcpkg triplet、静态/动态运行库和本库安装配置为准，架构与运行库必须匹配。
...
public:
    ...
    std::optional<curl_off_t> GetSize() const override;
};
```
...
```text
IsTransportSuccessful() -> 传输层是否成功
IsSuccessful()          -> 传输成功并且 HTTP 为 2xx
```
...
| 字段 | 声明/默认 | 作用 |
|---|---|---|
| `headers` | `HttpHeaders headers` | 请求头部。 |
| `rawHeaderLines` | `std::vector<std::string> rawHeaderLines` | 原生 HTTP 头部行。 |
| `cookies` | `HttpCookies cookies` | 显式请求 Cookie。 |
...
| `headers` | `HttpHeaders headers` | 结构化 Header。 |
...
| `originalHeaders` | `std::string originalHeaders` | 全部原始 Header 块。 |
| `headers` | `HttpHeaders headers` | 最终响应 Header，Trailer 也会合并到此处便于统一查询。 |
| `date` | `std::optional<std::chrono::system_clock::time_point> date` | 最终响应头中的Date。 |
| `trailers` | `HttpHeaders trailers` | HTTP Trailer 头部。 |
| `rawTrailers` | `std::string rawTrailers` | 原始 HTTP Trailer 文本。 |
| `cookies` | `HttpCookies cookies` | 最终响应 Set-Cookie。 |
| `curlCode` | `CURLcode curlCode = CURLE_OK` | 原始 CURLcode。 |
| `errorCategory` | `HttpErrorCategory errorCategory = HttpErrorCategory::None` | 封装层错误分类。 |
| `requestId` | `std::string requestId` | 请求追踪 ID。 |
| `finalRawHeaders` | `std::string finalRawHeaders` | 最终响应原始 Header。 |
...
| `Add()` | 追加参数，可控制是否保留 `=`。 |
| `Remove()` / `Has()` | 删除/判断参数。 |
| `GetSize()` / `IsEmpty()` / `Clear()` | 容器状态操作。 |
| `GetAllParameters()` / `GetAllParameterNames()` | 获取文本项/去重名称列表。 |
...
| `EraseHeader()` / `Contains()` | 删除/判断字段。 |
| `GetHeaderValue()` / `GetHeaderValues()` | 读取第一个/全部同名值。 |
| `GetKeys()` / `GetAllHeaders()` | 获取名称列表/序列化全部 Header。 |
| `GetItems()` / `begin/end` | 直接访问或遍历结构化 Header。 |
| `Clear()` / `IsEmpty()` / `GetSize()` | 容器状态操作。 |
...
| `EraseCookie()` / `Contains()` | 按名称或 Name/Domain/Path 身份删除 / 判断同名 Cookie。 |
...
| `IUploadSource::GetSize()` | 返回可选上传总大小。 |
| `MemoryUploadSource` | 用内存字符串实现 `IUploadSource`。 |
| `CallbackUploadSource` | 用业务回调实现 `IUploadSource`。 |
| `FileUploadSource` | 用本地文件实现 `IUploadSource`；`IsValid()` 检查打开状态，`GetPath()` 返回路径。 |
...
| `CurlTransport::IsValid()` | 检查 easy handle 是否有效。 |
| `CurlTransport::GetNativeHandle()` | 获取底层 `CURL*`；不得与 `Send()` 并发操作。 |
...
| `BasicMetricsCollector::GetSnapshot()` | 返回累计指标快照。 |
...
| 函数 | 作用 |
|---|---|
| `IsTransportSuccessful()` | `success && curlCode == CURLE_OK`，只判断传输层。 |
| `IsSuccessful()` | `IsTransportSuccessful()` 且最终 HTTP 状态为 2xx。 |
...
| `GetDefaultOptions()` | 读取默认请求配置。 |
| `SetClientOptions()` / `GetClientOptions()` | 设置/读取长期客户端策略。 |
| `SetTransport()` / `GetTransport()` | 替换/获取传输层，可注入 Mock。 |
...
| `GetPendingCount()` | 近实时等待/执行数量，仅用于观测，不是同步原语。 |
...
- 24 路 Async multi 并发与 `GetPendingCount()` 完成边界。
- 取消请求。
