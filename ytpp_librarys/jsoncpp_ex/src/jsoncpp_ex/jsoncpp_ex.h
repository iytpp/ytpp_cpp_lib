...
/// @param value 要读取、写入或处理的值。
/// @param commentStyle 传递给 Serialize 的 commentStyle 参数。
/// @param indentation 传递给 Serialize 的 indentation 参数。
/// @param enableYamlCompatibility 控制对应功能是否启用。
/// @param dropNullPlaceholders 传递给 Serialize 的 dropNullPlaceholders 参数。
/// @param useSpecialFloats 控制对应功能是否启用。
/// @param precision 传递给 Serialize 的 precision 参数。
/// @param precisionType 传递给 Serialize 的 precisionType 参数。
/// @param emitUtf8 传递给 Serialize 的 emitUtf8 参数。
...
/// @return 解析成功时返回true。
/// @param jsonText 传递给 Parse 的 jsonText 参数。
/// @param value 要读取、写入或处理的值。
...
/// @param jsonText 传递给 Parse 的 jsonText 参数。
/// @param value 要读取、写入或处理的值。
/// @param collectComments 控制对应功能是否启用。
...
/// @param jsonText 传递给 Parse 的 jsonText 参数。
/// @param value 要读取、写入或处理的值。
/// @param error 接收系统错误码；不需要时可传空指针。
/// @param collectComments 控制对应功能是否启用。
/// @param allowComments 控制对应功能是否启用。
/// @param allowTrailingCommas 控制对应功能是否启用。
/// @param strictRoot 传递给 Parse 的 strictRoot 参数。
/// @param allowDroppedNullPlaceholders 控制对应功能是否启用。
/// @param allowNumericKeys 控制对应功能是否启用。
/// @param allowSingleQuotes 控制对应功能是否启用。
/// @param stackLimit 对应的数量或限制值。
/// @param failIfExtra 传递给 Parse 的 failIfExtra 参数。
/// @param rejectDuplicateKeys 传递给 Parse 的 rejectDuplicateKeys 参数。
/// @param allowSpecialFloats 控制对应功能是否启用。
/// @param skipBom 传递给 Parse 的 skipBom 参数。
