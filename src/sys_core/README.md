...
## 头文件
```C++
#include "sys_core.h" //该头文件中包含了以下所有模块的头文件
using namespace ytpp::sys_core;
```

## 系统处理
所属头文件 `sys_processing.h`
### get_cursorPos [X | Y] 与 set_cursorPos [Orig] - 光标位置
```C++
/*
* @brief 设置光标位置
* @param [in] x: 水平位置
...
### get_screenWidth 与 get_screenHeight - 屏幕大小
```C++
/*
* @brief 获取屏幕宽度
* @return (int) 屏幕宽度
...
### write_profile 和 read_profile - 读写配置项
```C++
/*
...
### write_struct 和 read_struct - 读写结构体配置
```C++
/*
* @brief 写出结构体数据
* @param [in] fileName: 文件名
...
## 磁盘操作
所属头文件 `disk_manipulation.h`
### file_isExists - 文件是否存在
```C++
/*
* @brief 判断文件是否存在
